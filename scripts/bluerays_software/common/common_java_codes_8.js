...
function fetch_agent_details(agent_id,area_id){
	$('#agent_details_holder').css('height','500px');
...
function update_or_create_agent(agent_id){
	if($('#update_or_create_agent_button').html() != 'Wait...'){
		if($('#agent_name').val() == '' || $('#agent_name').val() == 'Enter agent name here'){
			...
			$('#new_agent_error_message').slideDown('fast');
			$('#new_agent_error_message').html('One of the phone numbers provided is being used by another agent. Please resolve this to proceed');
			$('#agent_phone').css('border-color','red');
		...
		}else if($('#agent_password').val() != $('#agent_password_2').val()){
			$('#new_agent_error_message').slideDown('fast');
...
function disable_agent(agent_id){
