	...
	for($oa=0;$oa<count($form_option_array);$oa++){
		...
		if(isset($this_option_index[0])){
			if($form_option_type_array[$oa] == 3 || $form_option_type_array[$oa] == 4){
				...
				if($option_value_array[$this_option_index[0]] == '-1'){
...
$this_data_set = mysqli_query($connect,"select * from $this_partition_name where id = $data_set_id")or die(mysqli_error($connect));
	...
	if(!isNaN(date_array[1])){
		if(date_array[0]){
			...
			if(date_array[1] < -15552000){
				$('#pharmacy_date_validation_status').html('Pharmacy date cannot be after 6 months');
				$('#pharmacy_date_validation_status').css('color','red');
				$('#pharmacy_date_validation_status').slideDown('fast');
				$('#pharmacy_date_holder').css('border','solid 1px red');
				$('#pharmacy_date_validation').val(0);
...
function check_if_clinical_future(){
	...
	if(!isNaN(date_array[1])){
		if(date_array[0]){
			...
			if(date_array[1] < -15552000){
				$('#clinical_date_validation_status').html('Clinical date cannot be after 6 months');
				$('#clinical_date_validation_status').css('color','red');
				$('#clinical_date_validation_status').slideDown('fast');
				$('#clinical_date_holder').css('border','solid 1px red');
				$('#clinical_date_validation').val(0);
...
<script>
function check_male_obstetric(){
	if($('#client_selected_sex').val() == 1){
		alert('Obstetric options are not applicable to males');
		
		$('#pregnant').click();		
		$('#option_137').val('');		
		$('#active_lmp_initiation_day').html('N/A');
		$('#active_lmp_initiation_month').html('N/A');
		$('#active_lmp_initiation_year').html('N/A');
		
		$('#active_edd_initiation_day').html('N/A');
		$('#active_edd_initiation_month').html('N/A');
		$('#active_edd_initiation_year').html('N/A');
...
<div style="width:100%;min-height:20px;height:auto;line-height:20px;float:left;"><div style="width:60%;min-height:20px;height:auto;float:left;">Are you currently pregnant?</div><div style="width:40%;height:20px;float:right;"><input <?php print($form_option_checked[136]);?> type="radio" id="field_136" name="pregnant" onclick="$('#option_136').val(1);calculate_edd();check_male_obstetric();"><label for="field_136">Yes </label><input type="radio" id="pregnant" name="pregnant" onclick="$('#option_136').val('');$('#active_edd_initiation_day').html('N/A');$('#active_edd_initiation_month').html('N/A');$('#active_edd_initiation_year').html('N/A');$('#option_138').val('');"><label for="pregnant">No</div></div>

<?php
if($form_option_value[136] == ''){
	$option_136 = '-1';
	
}else{
	$option_136 = $form_option_value[136];
}
?>


<input type="hidden" id="option_136" value="<?php print($option_136);?>">
...
if(date('m',time()) < 6){
	for($m=1;$m<13;$m++){
		...
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#initiation_edd_month_menu').toggle('fast');$('#active_edd_initiation_month').html($(this).html());$('#selected_edd_initiation_month').val(<?php print($m);?>);$('#option_138').val($('#selected_edd_initiation_month').val()+'/'+$('#selected_edd_initiation_day').val()+'/'+$('#selected_edd_initiation_year').val());check_male_obstetric()" style="width:40px;"><?php print($mo);?></div>
		<?php
	}
...
<div style="width:100%;min-height:20px;height:auto;line-height:20px;float:left;"><div style="width:60%;min-height:20px;height:auto;float:left;">Are you currently breastfeeding?</div><div style="width:40%;height:20px;float:right;"><input <?php print($form_option_checked[139]);?> type="radio" id="field_139" name="breastfeeding" onclick="$('#option_139').val(1);check_male_obstetric()"><label for="field_139">Yes </label><input type="radio" id="breastfeeding" name="breastfeeding" onclick="$('#option_139').val('');"><label for="breastfeeding">No</label></div></div>

<?php
if($form_option_value[139] == ''){
	$option_139 = '-1';
	
}else{
	$option_139 = $form_option_value[139];
}
?>

<input type="hidden" id="option_139" value="<?php print($option_139);?>">

<div style="width:100%;min-height:20px;height:auto;line-height:20px;float:left;"><div style="width:60%;min-height:20px;height:auto;float:left;">Client screen for CACX?</div><div style="width:40%;height:20px;float:right;"><input <?php print($form_option_checked[140]);?> type="radio" name="cacx" id="field_140" onclick="$('#option_140').val(1);check_male_obstetric()"><label for="field_140">Yes </label><input type="radio" id="cacx" name="cacx" onclick="$('#option_140').val('');"><label for="cacx">No</label></div></div>
...
<?php
if($form_option_value[765] == ''){
	$option_765 = '-1';
	
}else{
	$option_765 = $form_option_value[765];
	
}
?>
<script>
var site_name = $('#active_client_site').html();


var check_dreams = site_name.search('DREAM');

if(check_dreams != -1){
	$('#dreams_intervention').slideDown();
	$('#is_dreams_site').val(1);
	
}else{
	$('#is_dreams_site').val(0);
	
}

</script>
<input type="hidden" id="is_dreams_site" value="0">

<div style="width:100%;min-height:20px;height:auto;line-height:20px;float:left;display:none" id="dreams_intervention">
<div style="width:100%;height:20px;float:left;">DREAMS intervention</div><input <?php print($form_option_checked[765]);?> type="radio" id="field_765" name="dreams_intervention_field" onclick="$('#option_765').val(1);document.getElementById('field_765').checked = true;$('#option_765').val(1);"><label for="field_765">Yes </label><input type="radio" name="dreams_intervention_field" id="dreams_intervention_field" onclick="$('#option_765').val('');"><label for="dreams_intervention_field">No</label>
</div>
<input type="hidden" id="option_765" value="<?php print($option_765);?>">
</div>
...
<div style="width:100%;min-height:20px;height:auto;line-height:20px;float:left;"><input <?php print($form_option_checked[766]);?> type="radio" name="prep_dispensed" id="field_766" onclick="check_prep_dispensed(766);"><label for="field_766">6 Months</label></div>
...
<input type="hidden" value="<?php print($form_option_value[766]);?>" id="option_766">
...
<script>
	function check_prep_dispensed(field_id){
		...
		if($('#initiation_date_validation').val() == 0){
			$('#pharmacy_date_validation').val(0);
			$('#clinical_date_validation').val(0);
			
			$('#active_clinical_initiation_day').html('Select');
			$('#active_clinical_initiation_month').html('Select');
			$('#active_clinical_initiation_year').html('Select');
			
			$('#selected_clinical_initiation_day').val(0);
			$('#selected_clinical_initiation_month').val(0);
			$('#selected_clinical_initiation_year').val(0);
			
			$('#active_pharmacy_initiation_day').html('Select');
			$('#active_pharmacy_initiation_month').html('Select');
			$('#active_pharmacy_initiation_year').html('Select');
			
			$('#selected_pharmacy_initiation_day').val(0);
			$('#selected_pharmacy_initiation_month').val(0);
			$('#selected_pharmacy_initiation_year').val(0);
			...
			}else if(field_id == 217 || field_id == 218 || field_id == 219 || field_id == 766){
				if(field_id == 217){
					var multiplier = 30;
...
<div style="width:240px;height:min-120px;height:auto;float:left;padding:2px;">
...
<div style="width:auto;min-height:30px;height:auto;float:left;position:relative;z-index:50">
