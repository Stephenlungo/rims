...
mysqli_query($claims_connect,'use pipatzam_pipat_claims2')or die(mysqli_error($claims_connect));
	...
	$check_report_cache = mysqli_query($connect,"select * from dynamic_report_cache where dashboard_id = $dashboard_id and dashboard_area_id = $dashboard_area_id")or die(mysqli_error($connect));
	
	if(!mysqli_num_rows($check_report_cache)){
		$cached_report_found = 0;
		
	}else{
		$check_report_cache_results = mysqli_fetch_array($check_report_cache,MYSQLI_ASSOC);
		if((time() - $check_report_cache_results['_date']) >= 600){
			$cached_report_found = 0;
			
		}else{
			$cached_report_found = 1;

		}			
	}
	
	if($cached_report_found){
					...
					if($this_column_id == $primary_report_column_id_array[$pc]){
						...
						for($cr=0;$cr<count($column_rule_array);$cr++){
							...
							if($this_column_rule[0]){
								...
								$column_data = mysqli_query($$this_column_query,"select * from $this_column_table where company_id = $company_id $data_search_string $location_string $filter_script")or die(mysqli_error($$this_column_query).' - '.$this_column_id);
		...
		$entries = mysqli_query($$query_type,"select * from $reference_column where company_id = $company_id $filter_script")or die(mysqli_error($$query_type));
		...
		$entries = mysqli_query($$query_type,"select * from $reference_column where company_id = $company_id $search_string $filter_script")or die(mysqli_error($$query_type));
