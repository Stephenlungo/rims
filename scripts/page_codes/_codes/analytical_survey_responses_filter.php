...
<div style="width:auto;height:auto;float:left;">
	<div style="width:50px;height:30px;line-height:30px;float:left;">Survey:</div>
	<div style="width:auto;min-height:30px;height:auto;float:left;">
	
	<div class="option_item" title="Click to change option" onclick="$('#analytical_survey_menu').toggle('fast');" id="active_analytical_survey" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor='#eee';" style="min-width:110px;max-width:270px;width:auto;">Select survey</div>

	<div class="option_menu" id="analytical_survey_menu" style="display:none;min-width:120px;width:auto;">
	<?php
	$surveys = mysqli_query($connect,"select * from prep_questionnaires where module_id = 5 and company_id = $company_id order by title")or die(mysqli_error($connect));
	
	for($s=0;$s<mysqli_num_rows($surveys);$s++){
		$survey_results = mysqli_fetch_array($surveys,MYSQLI_ASSOC);
		?>
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#analytical_survey_menu').toggle('fast');$('#active_analytical_survey').html($(this).html());$('#selected_analytical_survey_id').val(<?php print($survey_results['id']);?>);"><?php print($survey_results['title']);?></div>
		<?php
	}
	?>
