...
<div style="width:100%;height:25px;float:left;border:solid 1px #eee;"><input type="checkbox" id="allow_future_check" onchange="if($('#allow_future_input').val() == 0){var c = confirm('Are you sure you wish to allow entry of future reports?');if(c){$('#allow_future_input').val(1);}else{document.getElementById('allow_future_check').checked = false;}}else{$('#allow_future_input').val(0);}"><label for="allow_future_check">Allow future dates</label></div>
<input type="hidden" id="allow_future_input" value="0">
...
<select id="basic_entry_day_0" onchange="if($('#basic_entry_day_0').val() >= <?php print(date('j',time()));?> && $('#basic_entry_month_0').val() == <?php print(date('m',time()));?> && $('#basic_entry_year_0').val() == <?php print(date('Y',time()));?> && $('#allow_future_input').val() == 0){alert('You cannot select a date for the future');this.value=<?php print(date('j',time()));?>}">
...
<select id="basic_entry_month_0" onchange="if($('#basic_entry_month_0').val() >= <?php print(date('m',time()));?> && $('#basic_entry_year_0').val() == <?php print(date('Y',time()));?> && $('#allow_future_input').val() == 0){alert('You cannot select a date for the future');this.value=<?php print(date('m',time()));?>}">
...
<div style="cursor:pointer;width:20px;height:25px;text-align:center;line-height:25px;color:#fff;float:right;background-color:#efe;" onmouseover="this.style.backgroundColor='#f55';" onmouseout="this.style.backgroundColor='brown';" title="Click to exit search" onclick="$('#new_entry_site_search_results_holder_0').slideUp('fast');">X</div>
</div>
