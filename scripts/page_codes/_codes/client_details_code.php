...
<?php
$dynamic_form_data_set_array = fetch_db_table('connect',$dynamic_form_data_set_table,1,'id',' client_id = '.$client_id);

$dynamic_forms = mysqli_query($connect,"select * from dynamic_forms where module_id = $module_id and company_id = $company_id and status = 1 order by _order")or die(mysqli_error($connect));
...
<div class="tab" style="min-height:25px;height:auto;line-height:25px;" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor=''" id="tab_11" onclick="if($('#client_screening_validated').val() == 0){alert('It appears you have not yet completed the eligibility screening. You need to complete the screening before proceeding');}else{$('#dynamic_form_holder').slideUp('fast');if($('#client_profile_updated').val() == 0){populate_profile_data(<?php print($client_date);?>);}$('#new_client_error_message').slideUp('fast');$('#client_update_holder').slideDown('fast');$('#client_profile').fadeIn('fast');$('#client_screening').hide();tab_item_change(11);<?php for($dd=0;$dd<mysqli_num_rows($dynamic_forms);$dd++){?> $('#dynamic_form_<?php print($dd);?>').hide();<?php }?>;change_window_size('item_details',820,500,1);}">Profile</div>
...
<div class="tab" style="min-height:25px;height:auto;line-height:25px;" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor=''" id="tab_<?php print(12 + $d);?>" onclick="check_form_dependencies(<?php print($dynamic_form_results['id'].','.(12 + $d));?>)"><?php print($dynamic_form_results['form_title']);?></div>

<?php

$data_set_index = array_keys($dynamic_form_data_set_array['dynamic_form_id'],$dynamic_form_results['id']);
...
function check_form_dependencies(form_id,form_index){
	...
	if(dependencies_passed==1){
		if($('#client_profile_validated').val() == 0){
			...
			fetch_dynamic_form_list(form_id,0,<?php print($client_date);?>);tab_item_change(form_index);change_window_size('item_details',1000,500,1);
		}
	}else{
...
if($client_id){
	...
	$reverse_index_id = $this_client_results['reverse_index_id'];
	
	$reverse_index_title = 'No';
	if($reverse_index_id){
		$reverse_index_title = 'Yes';
	}
		
	$population_category_id = $this_client_results['population_category_id'];
	
	if($population_category_id == -1){
		$population_category_title = 'Select item';
		
	}else if($population_category_id == 0){
	...
	}else if($population_category_id == 9){
		$population_category_title = 'Young Males';
		
	}else if($population_category_id == 10){
		$population_category_title = 'TG';
		
	}else if($population_category_id == 11){
		$population_category_title = 'PWID';
	...
	$inter_departmental_referal_title = 'None';
	$inter_departmental_referal_id = $this_client_results['inter_departmental_referal_id'];
	if($inter_departmental_referal_id == 1){
		$inter_departmental_referal_title = 'FP';
		
	}else if($inter_departmental_referal_id == 2){
		$inter_departmental_referal_title = 'ANC';
		
	}else if($inter_departmental_referal_id == 3){
		$inter_departmental_referal_title = 'VMMC';
		
	}else if($inter_departmental_referal_id == 4){
		$inter_departmental_referal_title = 'Index case Testing';
		
	}else if($inter_departmental_referal_id == 5){
		$inter_departmental_referal_title = 'OPD';
		
	}
	
	$reason_for_stopping = $this_client_results['reason_for_stopping'];
	if($reason_for_stopping == 0){
		$reason_for_stopping = '';
	}
	...
	$inter_departmental_referal_title = 'Select item';
	$inter_departmental_referal_id = '-1';
...
<div class="option_item" title="Click to change option" onclick="$('#population_category_menu').toggle('fast');" id="active_population_category" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor='#eee';" style="min-width:110px;max-width:280px;width:auto;"><?php print($population_category_title);?></div>
		...
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(6);population_category_validation();">Police Officer</div>
		
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(7);population_category_validation();">Inmates</div>
		
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(8);population_category_validation();">Prison Officer</div>
		
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(0);population_category_validation();">General</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(9);population_category_validation();">Young Males</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(1);population_category_validation();">MSM</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(2);population_category_validation();">DC</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(3);population_category_validation();">FSW</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(4);population_category_validation();">PLM</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(10);population_category_validation();">TG</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(5);population_category_validation();">AG/YW</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#population_category_menu').toggle('fast');$('#active_population_category').html($(this).html());$('#selected_population_category').val(11);population_category_validation();">PWID</div>
...
<script>
	function population_category_validation(){
		var this_pop_category = $('#selected_population_category').val();
		
		if(this_pop_category == 5){
			if(Number($('#client_age').val()) > 24){
				alert('Population category cannot be AGYW because the age for this client is above 24');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}else if($('#client_selected_sex').val() == 1){
				alert('Population category cannot be AGYW because this client is a male');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
				
			}else if(Number($('#client_age').val()) < 15){
				alert('Population category cannot be AGYW because age for this client is below 15');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}
		
		}else if(this_pop_category == 0){
			if($('#client_selected_sex').val() == 2 && Number($('#client_age').val()) >= 15 && Number($('#client_age').val()) <= 24){
				alert('Population category should be AGYW because the age for this client is between 15-24');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}else if($('#client_selected_sex').val() == 1 && Number($('#client_age').val()) >= 15 && Number($('#client_age').val()) <= 34){
				alert('Population category should be Young Males because the age for this client is between 15-34 and is a male');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
			}
			
		}else if(this_pop_category == 9){
			if($('#client_selected_sex').val() == 2){
				alert('Population category cannot be Young Males because the gender for this client is female');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}else if(Number($('#client_age').val()) < 15 || Number($('#client_age').val()) > 34){
				alert('Population category cannot be Young Males because the client is not in the 15-34 age band');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}
			
		}else if(this_pop_category == 1){
			if($('#client_selected_sex').val() == 2){
				alert('Population category cannot be MSM because the gender for this client is female');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
				
			}
			
		}else if(this_pop_category == 3 || this_pop_category == 4){
			if($('#client_selected_sex').val() == 1){
				alert('Population category cannot be PLM because the gender for this client is male');
				$('#selected_population_category').val('-1');
				$('#active_population_category').html('Select category');
...
<div style="width:100%;height:auto;float:left;margin-bottom:2px;">
<div style="width:140px;min-height:30px;height:auto;line-height:15px;float:left;">Inter-Departmental Referral:</div>
<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;">

<div class="option_item" title="Click to change option" onclick="$('#inter_departmental_referal_menu').toggle('fast');" id="active_inter_departmental_referal" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor='#eee';" style="min-width:110px;max-width:280px;width:auto;"><?php print($inter_departmental_referal_title);?></div>

		<div class="option_menu" id="inter_departmental_referal_menu" style="display:none;min-width:150px;max-width:280px;width:auto;">
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(1);">FP</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(2);">ANC</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(3);">VMMC</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(4);">Index Case Testing</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(5);">OPD</div>
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#inter_departmental_referal_menu').toggle('fast');$('#active_inter_departmental_referal').html($(this).html());$('#selected_inter_departmental_referal').val(0);">None</div>		
		</div>
	</div>
	<input type="hidden" id="selected_inter_departmental_referal" value="<?php print($inter_departmental_referal_id);?>">
</div>
...
<div class="option_item" title="Click to change option" onclick="$('#profile_client_status_menu').toggle('fast');" id="active_profile_client_status" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor='#eee';" style="min-width:110px;max-width:280px;width:auto;"><?php print($active_client_status_title);?></div>
		...
		<div class="option_menu" id="profile_client_status_menu" style="display:none;min-width:150px;max-width:280px;width:auto;">
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#profile_client_status_menu').toggle('fast');$('#active_profile_client_status').html($(this).html());$('#selected_profile_client_status').val(3);$('#new_client_error_message').slideUp('fast');$('#reason_for_stopping_holder').slideUp('fast');">Re-started</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#profile_client_status_menu').toggle('fast');$('#active_profile_client_status').html($(this).html());$('#selected_profile_client_status').val(6);$('#new_client_error_message').slideUp('fast');"$('#reason_for_stopping_holder').slideUp('fast');>No risk (Stopped)</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#profile_client_status_menu').toggle('fast');$('#active_profile_client_status').html($(this).html());$('#selected_profile_client_status').val(4);$('#new_client_error_message').slideUp('fast');$('#reason_for_stopping_holder').slideUp('fast');">Defaulted</div>
			
			<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#profile_client_status_menu').toggle('fast');$('#active_profile_client_status').html($(this).html());$('#selected_profile_client_status').val(5);$('#new_client_error_message').slideUp('fast');$('#reason_for_stopping_holder').slideDown('fast');">Stopped</div>
...
<div style="width:100%;height:auto;float:left;margin-bottom:2px;<?php if(!$client_id || $active_client_status_id != 5){print('display:none;');}?>" id="reason_for_stopping_holder">
<div style="width:140px;height:30px;line-height:30px;float:left;">Reason for stopping:</div>
<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;"><input type="text" style="width:100%;height:30px;" placeholder="Put reason for stopping here" value="<?php print($reason_for_stopping);?>" id="reason_for_stopping"></div>
...
<div style="width:100%;height:auto;float:left;margin-bottom:2px;">
<div style="width:140px;height:30px;line-height:30px;float:left;">Reverse index client:</div>
<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;">

<div class="option_item" title="Click to change option" onclick="$('#reverse_index_menu').toggle('fast');" id="active_reverse_index" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor='#eee';" style="min-width:110px;max-width:280px;width:auto;"><?php print($reverse_index_title);?></div>

		<div class="option_menu" id="reverse_index_menu" style="display:none;min-width:150px;width:auto;">
		
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#reverse_index_menu').toggle('fast');$('#active_reverse_index').html($(this).html());$('#selected_reverse_index').val(1);reverse_index_validation();">Yes</div>
		
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#reverse_index_menu').toggle('fast');$('#active_reverse_index').html($(this).html());$('#selected_reverse_index').val(0);reverse_index_validation();">No</div>
		
		
		</div>
	</div>
	<input type="hidden" id="selected_reverse_index" value="<?php print($reverse_index_id);?>">
</div>

</div>

<div style="width:100%;height:auto;float:left;margin-bottom:5px;" id="client_update_holder">
<div style="width:60px;height:30px;background-color:orange;color:#fff;text-align:center;line-height:30px;float:left;margin-right:5px;cursor:pointer;" onmouseover="this.style.backgroundColor='#ffc864';" onmouseout="this.style.backgroundColor='orange';"  id="client_profile_button" onclick="create_or_update_client(<?php print($client_date);?>);" title="Click to update account details">Update</div>
