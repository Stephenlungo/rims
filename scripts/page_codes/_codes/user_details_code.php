...
if($user_id){
	...
	$user_username = simple_encode($this_user_results['username']);
	$user_password = $this_user_results['password'];
	
	//$user_username = simple_decode($this_user_results['username']);

	//$user_password = simple_decode($this_user_results['password']);
	...
	$change_finance_processing = $roles[20];
	if($change_finance_processing){

		$change_finance_processing_check = ' checked ';
	}else{

		$change_finance_processing_check = '';		
...
<div class="tab" style="height:25px;line-height:25px;" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor=''" id="tab_10" onclick="$('#user_access_log').slideUp('fast');$('#user_basic_details').slideDown('fast');tab_item_change(10);">Basic details</div>

<div class="tab" style="height:25px;line-height:25px;" onmouseover="this.style.backgroundColor='#ddd';" onmouseout="this.style.backgroundColor=''" id="tab_11" onclick="$('#user_basic_details').slideUp('fast');$('#user_access_log').slideDown('fast');tab_item_change(11);">Access log</div>
...
<div style="width:100%;height:auto;float:left;display:none;margin-top:5px;" id="user_access_log">
<div style="width:100%;height:20px;line-height:20px;float:left;background-color:ddf;"><div style="width:100px;height:20px;float:left;">Date</div><div style="width:100px;height:20px;float:left;margin-right:3px;">Time</div><div style="width:170px;height:20px;float:left;">Action</div><div style="width:350px;height:20px;float:left;">Details</div></div>

<?php
	if($user_id){
		$access_log = mysqli_query($connect,"select * from user_access_log where user_id = $user_id order by _date desc")or die(mysqli_error($connect));
		...
		if(mysqli_num_rows($access_log)){
			...
			for($a=0;$a<mysqli_num_rows($access_log);$a++){
				...
				$color = '#000';
				if($access_log_results['entry_type'] == 0){
					$entry_action = 'Unrecognised user';
					
				}else if($access_log_results['entry_type'] == 1){
					$entry_action = 'Password expired';
					
				}else if($access_log_results['entry_type'] == 2){
					$entry_action = 'Login successful...';
					
				}else if($access_log_results['entry_type'] == 3){
					$entry_action = 'Login attempts exceeded';
					
				}else if($access_log_results['entry_type'] == 4){
					$entry_action = 'Invalid password';
					$color = 'brown';
				}
				
				?>
				<div style="width:100%;color:<?php print($color);?>;min-height:20px;height:auto;line-height:20px;float:left;border-bottom:solid 1px #eee;"><div style="width:100px;min-height:20px;height:auto;float:left;"><?php print(date('jS M, Y',$access_log_results['_date']));?></div><div style="width:100px;min-height:20px;height:auto;float:left;"><?php print(date('H:i:s',$access_log_results['_date']));?></div><div style="width:170px;min-height:20px;height:auto;float:left;margin-right:3px;"><?php print($entry_action);?></div><div style="width:350px;min-height:20px;height:auto;float:left;margin-right:3px;"><?php print($access_log_results['details']);?></div></div>				
				<?php
			}			
		}
...
<div style="width:100%;height:auto;float:left;margin-top:5px;" id="user_basic_details">
...
<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;"><input type="<?php if(!$editing){print('password');}else{print('text');}?>" style="width:100%;border:solid 1px #aaa;height:30px;color:#000" value="<?php print($user_username);?>"  id="user_username" <?php if(!$editing){?> disabled<?php }?> onfocusout="if(this.value==''){this.value='<?php print(time());?>';this.style.color='#000'}" onfocus="$('#new_user_error_message').slideUp('fast');this.style.borderColor='#aaa';" onchange="check_password_and_username(<?php print($user_id);?>);"></div>


<input type="hidden" id="username_passed" value="0">

<div style="width:100%;height:auto;float:left;margin-bottom:5px;margin-top:5px;">

<div style="width:130px;height:30px;line-height:30px;float:left;margin-left:10px;">New Password*:</div>

<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;"><input type="password" style="width:100%;border:solid 1px #aaa;height:30px;color:#000" value="password"  id="user_password" <?php if(!$editing){?> disabled<?php }?> onfocusout="if(this.value==''){this.value='password';}" onfocus="$('#new_user_error_message').slideUp('fast');this.style.borderColor='#aaa';$('#user_password').css('border-color','#aaa');if(this.value=='password'){this.value=''}" onchange="check_password_and_username(<?php print($user_id);?>);"></div>

</div>
<input type="hidden" id="password_passed" value="0">
...
<div style="width:230px;min-height:30px;height:auto;float:left;line-height:30px;"><input type="password" style="width:100%;border:solid 1px #aaa;height:30px;color:#000" value="password"  id="user_password_2" <?php if(!$editing){?> disabled<?php }?> onfocusout="if(this.value==''){this.value='password'}" onfocus="$('#new_user_error_message').slideUp('fast');this.style.borderColor='#aaa';$('#user_password').css('border-color','#aaa');$('#user_password_2').css('border-color','#aaa');if(this.value=='password'){this.value=''}"></div>

</div>


<div style="width:100%;min-height:30px;height:auto;line-height:30px;float:left;display:none;" id="login_check_status"></div>
</div>
...
<div style="width:100%;height:20px;float:left;background-color:#fee;text-align:center;font-weight:bold;">User Access Restrictions</div>
...
<input type="checkbox" <?php print($change_finance_processing_check); if(!$active_user_roles[20] || !$roles[9]){print(' disabled ');}?> id="change_finance_processing" onchange="if(this.checked){$('#change_finance_processing_input').val(1);}else{$('#change_finance_processing_input').val(0);}" <?php if(!$editing){?> disabled<?php }?>> <label for="change_finance_processing">Allow changing finance processing</label><br>

<input type="hidden" id="change_finance_processing_input" value="<?php print($change_finance_processing);?>">
...
<div style="width:100%;height:auto;float:left;margin-bottom:5px;" id="user_update_holder">
	...
	if($user_id and $active_user_roles[8] and $settings_results['url_password_reset']){
		?>
		<div style="width:170px;height:30px;background-color:#c27cc2;color:#fff;text-align:center;line-height:30px;float:left;margin-right:5px;cursor:pointer;" onmouseover="this.style.backgroundColor='#d193d1';" onmouseout="this.style.backgroundColor='#c27cc2';"  id="password_reset_link_button" onclick="send_password_reset_link(<?php print($user_id);?>);" title="Click to proceed">Send password reset link</div>
<?php
	}
...
<script>
check_password_and_username(<?php print($user_id);?>);
</script>
