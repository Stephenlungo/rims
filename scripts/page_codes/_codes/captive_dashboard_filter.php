...
<div style="width:40px;height:30px;line-height:30px;float:left;">WI-FI:</div>
		...
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#wifi_menu').toggle('fast');$('#active_wifi').html($(this).html());$('#selected_wifi').val(0);$('#active_survey').html('All surveys');fetch_menu_items('connect','prep_questionnaires','module_id',4,'survey',1,1,'');create_filter_script()">All WIFI's</div>
		...
		<div style="width:auto;height:auto;float:left;">
		<div style="width:45px;height:30px;line-height:30px;float:left;">Survey:</div>
		<div style="width:auto;min-height:30px;height:auto;float:left;" onclick="$('#new_user_error_message').hide('fast');">
		...
		<div class="option_menu" id="survey_menu" style="display:none;width:auto;" onclick="create_filter_script()">
		
		<div class="option_menu_item" onmouseover="this.style.color='#006bb3';" onmouseout="this.style.color='#000';" onclick="$('#survey_menu').toggle('fast');$('#active_survey').html($(this).html());$('#selected_survey').val(0);create_filter_script()">All surveys</div>
...
function create_filter_script(){
	if($('#selected_wifi').val() == 0){
		var wifi_filter = '';
		
	}else{
		var wifi_filter = ' and wifi_id = '+$('#selected_wifi').val();
	...
	if($('#selected_survey').val() == 0){
		...
		var survey_filter = ' and questionnaire_id = '+$('#selected_survey').val();
		
	}

	$('#filter_script').val(wifi_filter+survey_filter);
}
