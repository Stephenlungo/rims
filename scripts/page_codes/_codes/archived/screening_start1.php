...
<div style="width:100%;height:auto;float:left;margin-top:30px;text-align:center;">Thank you for visiting PrEP screening. Please select a questionnaire below:<br></div>

<div style="width:100%;height:auto;float:left;margin-top:5px;">
<?php
$questionnaires = mysqli_query($connect,"select * from prep_questionnaires where module_id = 3 and status = 1 order by title")or die(mysqli_error($prep_connect));
...
for($q=0;$q<mysqli_num_rows($questionnaires);$q++){
	...
	if($client_id){
		...
		$check_client_answers = mysqli_query($connect,"select * from prep_client_answers where client_id = $client_id and questionnaire_id = $questionnaire_id")or die(mysqli_error($connect));
		...
		if(mysqli_num_rows($check_client_answers)){
			...
			$function_code = 'fetch_screening_results('.$questionnaire_id.','.$client_id.')';
