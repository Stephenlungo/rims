...
<?php
$dynamic_form_data_set_array = fetch_db_table('connect','dynamic_form_data_sets',1,'id',' client_id = '.$client_id);
...
function check_form_dependencies(form_id,form_index){
	...
	if(dependencies_passed==1){
		if($('#client_profile_validated').val() == 0){
			...
			fetch_dynamic_form_list(form_id,0);tab_item_change(form_index);change_window_size('item_details',1000,500,1);
		}
	}else{
...
if($client_id){
	...
	$inter_departmental_referal_title = 'None';
	$inter_departmental_referal_id = 0;
}
?>
