...
<?php
if($this_form_results['data_processing_method'] == 0){
...
<?php
$dynamic_forms = mysqli_query($connect,"select * from dynamic_forms where company_id = $company_id and id != $form_id and status = 1")or die(mysqli_error($connect));

for($d=0;$d<mysqli_num_rows($dynamic_forms);$d++){
	$dynamic_form_reults = mysqli_fetch_array($dynamic_forms,MYSQLI_ASSOC);
	
	if(check_item_in_list($dynamic_form_reults['id'],$this_form_results['dependencies'],0,',')){
...
<input type="hidden" id="form_dependecies" value="<?php print($this_form_results['dependencies']);?>">
</div>
