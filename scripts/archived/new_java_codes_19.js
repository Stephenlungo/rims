...
module_xmlhttp.onreadystatechange = function(){
	if(module_xmlhttp.readyState == 4 && module_xmlhttp.status == 200){
		...
		if(response_array[0] == 'create_or_update_branch'){
			...
			display_infor('activity_log_holder',response_array[1]);
		...
		}else if(response_array[0] == 'fetch_covid_client_list' || response_array[0] == 'fetch_covid_ussd' || response_array[0] == 'fetch_covid_whatsapp' || response_array[0] == 'fetch_covid_questionnaire_list'){
...
function ussd_activity_log(){
	...
	show_loading_progress('activity_log_holder','Loading. One moment please...');
}
