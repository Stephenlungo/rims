...
module_xmlhttp.onreadystatechange = function(){
	if(module_xmlhttp.readyState == 4 && module_xmlhttp.status == 200){
		...
		if(response_array[0] == 'create_or_update_branch'){
			...
			$('#send_offline_pipat_button').html('Download Offline System Version');
			
			if(response_array[1] == 1){
				alert('File sent to your email successfully...');
...
function create_new_prep_report(report_id){
	...
	$('#item_details_title').html('PrEP Custom report');
	...
	if($('#group_update_button').html() != 'Wait...'){
		if($('#group_title').val() == 'Enter title here'){			
			...
			if(c){
				...
				data.append('schedule_id',$('#selected_group_schedule').val());
