...
module_xmlhttp.onreadystatechange = function(){
	if(module_xmlhttp.readyState == 4 && module_xmlhttp.status == 200){
		...
		if(response_array[0] == 'create_or_update_branch'){
		...
		}else if(response_array[0] == 'ussd_activity_log'){
			display_infor('ussd_holder',response_array[1]);
			
		}else if(response_array[0] == 'fetch_sms_queue' || response_array[0] == 'fetch_sms_clients' || response_array[0] == 'fetch_sms_client_groups'){
			display_infor('ussd_holder',response_array[1]);
		...
		}else if(response_array[0] == 'process_prep_report'){
			$('#process_prep_report_button').html('Process report');
			window.open($('#url').val()+'/'+response_array[1],'file_download');
			...
			fetch_dynamic_form_list(response_array[1],1,response_array[2]);
			...
			display_infor('item_details_'+response_array[1]+'_holder',response_array[2]);
		...
		}else if(response_array[0] == 'create_new_prep_report'){
			display_infor('item_details_holder',response_array[1]);
			
		}else if(response_array[0] == 'save_prep_custom_report' || response_array[0] == 'create_new_ussd_report'){
			display_infor('item_details_holder_1',response_array[1]);
			
		}else if(response_array[0] == 'process_save_prep_report_2'){
			close_window('item_details_1');
			close_window('item_details');
			
			if(response_array[1] != 0){
				$('#quick_report_item_'+response_array[1]).remove();
				$('#active_quick_report').html(response_array[3]);
				
			}
			
			$('#screening_rule_string').val(response_array[4]);
			$('#profile_rule_string').val(response_array[5]);
			$('#form_rule_string').val(response_array[6]);
			
			$('#quick_report_menu').append('<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#quick_report_menu\').toggle(\'fast\');$(\'#active_quick_report\').html($(this).html());$(\'#selected_quick_report\').val('+response_array[2]+');$(\'#screening_rule_string\').val(\''+response_array[4]+'\');$(\'#profile_rule_string\').val(\''+response_array[5]+'\');$(\'#form_rule_string\').val(\''+response_array[6]+'\');create_new_prep_report('+response_array[2]+');" id="quick_report_item_'+response_array[2]+'">'+response_array[3]+'</div>');
			
		}else if(response_array[0] == 'delete_prep_report'){
			$('#active_quick_report').html('Select report');
			$('#quick_report_item_'+response_array[1]).remove();
			
			$('#screening_rule_string').val('');
			$('#profile_rule_string').val('');
			$('#form_rule_string').val('');
			$('#selected_quick_report').val(0);
		...
		}else if(response_array[0] == 'process_error_report'){
			$('#report_error_report_button').html('Fetch report');
			window.open($('#url').val()+'/'+response_array[1],'file_download');
			
		}else if(response_array[0] == 'validate_follow_up_date'){
			if(response_array[1] == 1){
				$('#date_validation_status').html('<div style="width:100%;height:100%;float:left;text-align:center;color:green">Date validation passed</div>');
				
			}else{
				if(response_array[1] == 0){
					$('#date_validation_status').html('<div style="width:100%;height:100%;float:left;text-align:center;color:red">There is another form on this date</div>');
					
				}else if(response_array[1] == 3){
					$('#date_validation_status').html('<div style="width:100%;height:100%;float:left;text-align:center;color:red">Follow-up date is in the future</div>');
					
				}else{
					$('#date_validation_status').html('<div style="width:100%;height:100%;float:left;text-align:center;color:red">Date provided is earlier than initiation date</div>');
				}				
			}
			
			$('#follow_up_date_validated').val(response_array[1]);
			
			check_if_pharmacy_future();
			check_if_clinical_future();
			
		}else if(response_array[0] == 'import_multiple_clients'){
			$('#item_details_'+response_array[1]+'_holder').html(response_array[2]);
			
		}else if(response_array[0] == 'send_offline_pipat'){
			$('#send_offline_pipat_button').html('Download offline version');
			...
			if(response_array[1] == 1){
				...
				alert('Your account has no email confingured. Add an email to your account');
				
			}else if(response_array[1] == 2){
				alert('You are not authorised to download an offline version of the system');
				
			}
			
		}else if(response_array[0] == 'fetch_covid_client_list' || response_array[0] == 'fetch_covid_ussd' || response_array[0] == 'fetch_covid_whatsapp' || response_array[0] == 'fetch_covid_questionnaire_list' || response_array[0] == 'fetch_tempreture_scanner'){
			display_infor('clients',response_array[1]);
			
		}else if(response_array[0] == 'update_covid_form_client_profile'){
			$('#dynamic_form_save_button_'+response_array[2]).html('Update');
			create_or_update_custom_client_form(response_array[2],response_array[3],1);
			
		}else if(response_array[0] == 'fetch_stories_list'){
			display_infor('stories',response_array[1]);
			
		}else if(response_array[0] == 'update_agent_type'){
			...
			$('#agent_'+response_array[1]).css('background-color',bg_color);
			$('#agent_'+response_array[1]).attr('onmouseover',"this.style.backgroundColor='"+hover_color+"'");
			$('#agent_'+response_array[1]).attr('onmouseout',"this.style.backgroundColor='"+bg_color+"'");
			
			alert('Agent updated successfully...');
			
		}else if(response_array[0] == 'export_tempreture_scanner'){
			window.open($('#url').val()+'/'+response_array[1],'file_download');
			
			$('#tempreture_export_button').html('Export');
			
		}else if(response_array[0] == 'fetch_partition_details'){
			display_infor('item_details_holder',response_array[1]);
		
			
		}else if(response_array[0] == 'update_system_settings'){
			alert('Settings updated successfully...');
			$('#update_or_create_settings_button').html('Update');
			
		}else if(response_array[0] == 'check_password_and_username'){
			$('#username_passed').val(0);
			if(response_array[1] == 0){
				$('#username_passed').val(1);		
			}
			
			$('#password_passed').val(0);
			if(response_array[2] == 0){
				$('#password_passed').val(1);		
			}
			
			if(response_array[1] == 1){
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:red;">Minimum user-name length must be '+response_array[3]+'</div>');
				
			}else if(response_array[1] == 2){
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:red;">There is another user with this username. Please try a different user name</div>');				
			
			}else if(response_array[2] == 1){
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:red;">Minimum password length must be '+response_array[4]+'</div>');				
				
			}else if(response_array[2] == 2){
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:red;">Password needs to have at-least one capital letter</div>');
				
			}else if(response_array[2] == 3){
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:red;">Password needs to have at-least one special charactor (!,# or ?)</div>');
				
			}else{
				$('#login_check_status').html('<div style="100%;min-height:20px;height:auto;float:left;line-height:20px;color:green;">Login check passed...</div>');				
			}
			
			$('#login_check_status').slideDown('fast');
			
		}else if(response_array[0] == 'allocate_agents'){
			display_infor('item_details_holder',response_array[1]);
			
		}else if(response_array[0] == 'search_allocation_users'){
			if(response_array[1] == 0){
				$('#search_results_holder').html(response_array[2]);
				
			}else{
				$('#search_results_holder1').html(response_array[2]);
				
			}
			
		}else if(response_array[0] == 'complete_allocate_user'){
			alert('Operation successfuly...');
			close_window('item_details',1);
			
			fetch_agent_validation_list();
			
		}else if(response_array[0] == 'update_or_create_application'){
			close_window('item_details');
			$('#tab_8').click();
			
		}else if(response_array[0] == 'send_password_reset_link'){
			if(response_array[1] == 1){
				alert('Password reset link was not sent. '+response_array[2]);
			}else{
				alert('Password reset link sent successfully...');
			}
			
			$('#password_reset_link_button').html('Send password reset link');
			
		}else if(response_array[0] == 'check_other_partition_date'){
			if(response_array[1] ==0){
				$('#period_from_validation_status').html('Another partition exists in selected period. You cannot have two partitions in the same or overlapping periods');
				
				$('#period_from_validation_status').css('color','red');
				
			}else{
				$('#period_from_validation_status').html('Period validation successful...');
				$('#period_from_validation_status').css('color','green');
			}
			
			$('#period_from_validity').val(response_array[1]);
			
		}else if(response_array[0] == 'update_or_create_partition'){
			close_window('item_details');
			
			$('#tab_5').click();
			
		}else if(response_array[0] == 'fetch_activity_calendar_list'){
			$('#activity_calendar_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'fetch_calendar_activity'){
			$('#item_details_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'create_or_update_activity'){
			close_window('item_details');
			fetch_activity_calendar_list(0);
			
		}else if(response_array[0] == 'add_activity_budget'){
			$('#item_details_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'process_add_activity_budget'){
			close_window('item_details');
			fetch_activity_calendar_list(active_tab);
			
		}else if(response_array[0] == 'reorganise_data_sets'){
			$('#tab_'+response_array[1]).click();
			
		}else if(response_array[0] == 'fetch_meetings' || response_array[0] == 'fetch_meeting_participants' || response_array[0] == 'fetch_meeting_batches'){
			$('#meeting_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'check_meeting_code'){
			$('#meeting_check_holder_details').html(response_array[1]);
			
			$('#meeting_code_check_button').html('Check');
			
		}else if(response_array[0] == 'view_meeting_details'){
			$('#item_details_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'create_or_update_meeting'){
			close_window('item_details');
			fetch_meetings();
			
		}else if(response_array[0] == 'approve_meeting_participants'){
			$('#meeting_expanded_'+response_array[1]).val(0);
			expand_collapse_meeting(response_array[1]);
			
			$('#approve_meeting_participants_button_'+response_array[1]).html('Approve selected');
			$('#approve_meeting_participants_button_1_'+response_array[1]).html('Approve selected');
			
			$('#disapprove_meeting_participants_button_'+response_array[1]).html('Disapprove selected');
			$('#disapprove_meeting_participants_button_1_'+response_array[1]).html('Disapprove selected');
			
		}else if(response_array[0] == 'confirm_move_meeting_participants'){
			close_window('item_details');
			$('#meeting_expanded_'+response_array[1]).val(0);
			expand_collapse_meeting(response_array[1]);
			
		}else if(response_array[0] == 'delete_meeting_participants'){
			$('#meeting_expanded_'+response_array[1]).val(0);
			expand_collapse_meeting(response_array[1]);
			
			$('#delete_meeting_participants_button_'+response_array[1]).html('Delete selected');
			$('#delete_meeting_participants_button_1_'+response_array[1]).html('Delete selected');
			
		}else if(response_array[0] == 'export_meeting_list'){
			window.open($('#url').val()+'/'+response_array[1],'file_download');
			
			$('#export_meeting_button').html('Export meetings');
			$('#export_participant_button').html('Export participants');
			
		}else if(response_array[0] == 'fetch_ussd_loader'){
			$('#ussd_response').html(response_array[2]);
			
			$('#ussd_string').val('');
			
			document.getElementById('c_id').disabled = true;
			
			if(response_array[1] == 0 || response_array[1] == '573'){
				document.getElementById('c_id').disabled = false;
			}
			
		}else if(response_array[0] == 'export_participant_list'){
			$('#export_meeting_button').html('Export meetings');
			$('#export_participant_button').html('Export participants');
			
		}else if(response_array[0] == 'batch_meetings'){
			$('#item_details_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'process_batch_meetings'){
			close_window('item_details');
			
			$('#selected_meetings').val('');
			fetch_meetings();
			
		}else if(response_array[0] == 'delete_meeting_batch'){
			$('#batch_item_'+response_array[1]).remove();
			
		}else if(response_array[0] == 'edit_batch'){
			$('#item_details_holder').html(response_array[1]);
			
		}else if(response_array[0] == 'update_batch'){
			close_window('item_details');
			fetch_meeting_batches();
			
		}else if(response_array[0] == 'process_batch_attachment'){
			var uploaded_files = $('#uploaded_files').val();
			var uploaded_files_array = uploaded_files.split(',');
			
			var total_batch_files = Number($('#total_batch_files_'+response_array[1]).val());
			
			total_batch_files += Number(uploaded_files_array.length);
			
			var output_string = '';
			for(var f=0;f<uploaded_files_array.length;f++){
				output_string += '<div style="width:auto;padding:2px;margin:2px;float:left;height:30px;line-height:30px;background-color:#d9d2e9;border-radius:20px;overflow:hidden" id="batch_file_'+response_array[1]+'_'+(f+total_batch_files)+'" onmouseover="this.style.backgroundColor=\'#baaed6\'" onmouseout="this.style.backgroundColor=\'#d9d2e9\'"><div style="cursor:pointer;width:auto;float:left;user-select:none;" ondblclick="window.open($(\'#url\').val()+\'/imgs/'+uploaded_files_array[f]+'\',\'file\')">'+uploaded_files_array[f]+'</div><div style="margin-left:2px;text-align:center;cursor:pointer;width:20px;height:30px;line-height:30px;float:right;background-color:brown;color:#fff;" onclick="remove_batch_file('+response_array[1]+',\''+uploaded_files_array[f]+'\')">X</div></div>';
				
			}
			
			if($('#total_batch_files_'+response_array[1]).val() == 0){
				$('#batch_files_'+response_array[1]).html(output_string);
				
			}else{
				$('#batch_files_'+response_array[1]).append(output_string);
			}
			
			
			
			$('#total_batch_files_'+response_array[1]).val(total_batch_files);
			
			$('#close_uploader_button').click();
			
		}else if(response_array[0] == 'remove_batch_file'){
			$('#batch_file_'+response_array[1]+'_'+response_array[2]).remove();
			
		}else if(response_array[0] == 'send_batch_to_finance'){
			$('#batch_item_'+response_array[1]).remove();
			
		}else if(response_array[0] == 'fetch_payment_folders'){
			$('#agents').html(response_array[1]);
			
		}else if(response_array[0] == 'fetch_payment_folder_content'){
			$('#payment_folder_holder').html(response_array[4]);
			
			var new_current_level = Number(response_array[2]);
			var current_level = Number($('#folder_level').val());
			
			if(current_level != new_current_level){
				for(var l=new_current_level;l<50;l++){
					if(l != 0){
						if($('#level_folder_'+l).html() == undefined){
							break;
							
						}else{
							$('#level_folder_'+l).remove();
				...
				if(response_array[1] != 0){
					...
					var level_div = '<div style="padding-left:5px;padding-right:5px;margin-left:5px;min-width:70px;width:auto;height:30px;float:left;text-align:center;background-color:#dde;border-radius:0px 20px 20px 0px;cursor:pointer;display:none;" onclick="fetch_payment_folder_content('+response_array[1]+','+new_current_level+')" onmouseover="this.style.backgroundColor=\'#ccd\';" onmouseout="this.style.backgroundColor=\'#dde\';" id="level_folder_'+(new_current_level)+'">'+response_array[3]+'</div>';
					
					$('#folder_level_holder').append(level_div);
					
					$('#level_folder_'+(new_current_level)).fadeIn('fast');
					$('#level_folder_'+(new_current_level)).css('backgroundColor','#ded');
					$('#level_folder_'+(new_current_level)).attr('onmouseover',"this.style.backgroundColor='#cdc'");
					$('#level_folder_'+(new_current_level)).attr('onmouseout',"this.style.backgroundColor='#ded'");
					
					
					$('#level_folder_'+(current_level)).css('backgroundColor','#dde');
					$('#level_folder_'+(current_level)).attr('onmouseover',"this.style.backgroundColor='#ccd'");
					$('#level_folder_'+(current_level)).attr('onmouseout',"this.style.backgroundColor='#dde'");
					
					$('#payment_folder_edit_options').slideDown('fast');
					
					$('#edit_payment_folder_button').attr("onclick","fetch_payment_folder_details("+response_array[1]+")");
					
				}else{
					$('#payment_folder_edit_options').slideUp('fast');
					
				}
				
				$('#current_folder_id').val(response_array[1]);
				$('#folder_level').val(new_current_level);
			}
			
		}else if(response_array[0] == 'fetch_payment_folder_details'){
			$('#item_details_holder').html(response_array[2]);
			
			if(response_array[1] == 0){
				$('#active_folder_region').html($('#active_region').html());
				$('#active_folder_province').html($('#active_province').html());
				$('#active_folder_hub').html($('#active_hub').html());
				$('#active_folder_site').html($('#active_site').html());
				
				$('#selected_folder_region').val($('#selected_region').val());
				$('#selected_folder_province').val($('#selected_province').val());
				$('#selected_folder_hub').val($('#selected_hub').val());
				$('#selected_folder_site').val($('#selected_site').val());
			}
			
		}else if(response_array[0] == 'save_or_update_payment_folder'){
			close_window('item_details');
			$('#level_folder_'+(Number(response_array[3])-1)).click();
			
		}else if(response_array[0] == 'delete_payment_folder'){
			var folder_level = Number($('#folder_level').val());
			
			$('#level_folder_'+(folder_level-1)).click();
			
			close_window('item_details');
			
		}else if(response_array[0] == 'add_agents_to_folder'){
			$('#item_details_holder').html(response_array[3]);
			
		}else if(response_array[0] == 'search_folder_agents'){
			$('#agent_folder_search_results').html(response_array[1]);
			
		}else if(response_array[0] == 'batch_folder_agents'){
			$('#item_details_holder').html(response_array[2]);
			
		}else if(response_array[0] == 'save_add_agents_to_folder'){
			close_window('item_details');
			$('#level_folder_'+$('#folder_level').val()).click();
			
		}else if(response_array[0] == 'save_folder_claim_data'){
			$('#save_folder_claim_data_button').html('Save');
			alert('Claim settings for this folder have been saved');
			
		}else if(response_array[0] == 'create_claim_from_folder'){
			alert('Claim created successfuly...');
			
			fetch_payment_folder_details($('#current_folder_id').val(),1);
			
		}else if(response_array[0] == 'restore_folder_claim_beneficiaries'){
			alert('Beneficiaries for this claim type have been restored...');	

			$('#restore_beneficiaries_button_'+response_array[1]).html('Restore beneficiaries on claim');		
...
function fetch_detailed_list(){
	if($('#last_entry_id').val() != undefined && $('#editing_active').val() == 0){
		...
		data.append('database_id',$('#selected_database').val());
...
function prepare_ussd_activity_log(){
	fetch_script('_codes/ussd_activity_log.php?a=0','ussd_holder');
...
function fetch_ussd_menu_details(menu_id){
	$('#item_details_holder').css('height','400px');
...
function create_or_update_ussd_menu_item(menu_id,parent_id,level){	
	if($('#create_or_edit_menu_button').html() != 'Wait...'){
		...
		if($('#ussd_menu_title').val() == 'Enter title here' || $('#ussd_menu_title').val() == ''){
			...
			data.append('_style',$('#new_ussd_menu_style').val());
		...
		if($('#update_or_create_site_button').html() != 'Wait...'){
			if($('#site_title').val() == '' || $('#site_title').val() == 'Enter title for this site here'){
				...
				if(c){
					...
					data.append('gsm_code',$('#gsm_code').val());
					data.append('gsm_code_2',$('#gsm_code_2').val());
			...
			if(error){
				...
				for(var s=0;s<total_sessions;s++){
					...
					for(var q = 0;q<total_questions;q++){
						...
						data.append('question_mandatory_'+s+'_'+q,$('#question_mandatory_'+s+'_'+q).val());
						data.append('dependancy_string_'+s+'_'+q,$('#selected_question_dependency_ession_'+s+'_'+q).val()+']'+$('#selected_question_dependency_question_'+s+'_'+q).val()+']'+$('#selected_question_dependency_option_'+s+'_'+q).val());
...
function fetch_client_details(client_id,form_ind,client_date){
	var data = new FormData();
	...
	data.append('to_month',$('#selected_to_month').val());
	data.append('to_year',$('#selected_to_year').val());
	data.append('client_date',client_date);
	show_window('item_details',1);
...
function fetch_covid_client_details(client_id){
	var data = new FormData();
	data.append('fetch_covid_client_details',1);
	data.append('client_id',client_id);
	show_window('item_details',1);
	change_window_size('item_details',400,500,1);
	...
	show_loading_progress('item_details_holder','Preparing. One moment...');
	$('#item_details_title').html('COVID-19 Client Account');
	$('#item_details_title_bar').css('background-color','brown');
	
	$('#details_close_button').attr('onclick',"close_window('item_details');");
}

function fetch_dynamic_form(form_id,module_id){
	var data = new FormData();
...
function create_or_update_client(client_date){
	if($('#client_profile_button').html() != 'Wait...'){
		var error = 0;
		if($('#client_screening_validated').val() == 1){
			if($('#client_name').val() == '' || $('#client_name').val() == 'Enter client name here' || $('#client_name').val() == 'Fetching. Wait...'){
			...
			}else if($('#selected_population_category').val() == -1){
				$('#new_client_error_message').slideDown('fast');
				$('#new_client_error_message').html('Select population category for this client');			
				
				error = 1;
				
			}else if($('#selected_inter_departmental_referal').val() == -1){
				$('#new_client_error_message').slideDown('fast');
				$('#new_client_error_message').html('You need to select the inter-departmental referal');
				
				error = 1;
				
			}else if($('#selected_profile_client_status').val() == 5 && $('#reason_for_stopping').val() ==''){
				$('#new_client_error_message').slideDown('fast');
				$('#new_client_error_message').html('You need to reason for stopping');
		...
		if(!error){
			...
			data.append('reverse_index_id',$('#selected_reverse_index').val());
			...
			data.append('inter_departmental_referal_id',$('#selected_inter_departmental_referal').val());
			data.append('reason_for_stopping',$('#reason_for_stopping').val());
...
function create_or_update_covid_client(){
	//if($('#client_profile_button').html() != 'Wait...'){
		var error = 0;
		...
		}else if($('#selected_case_classification').val() == -1){
			$('#new_client_error_message').slideDown('fast');
			$('#new_client_error_message').html('Select case classification for this client');			
		...
		if(!error){
			var data = new FormData();
			data.append('create_or_update_covid_client',1);
			data.append('tmp_client_id',$('#tmp_client_id').val());
			data.append('client_id',$('#client_id').val());
			data.append('client_name',$('#client_name').val());
			data.append('client_phone',$('#client_phone').val());
			...
			data.append('agent_id',$('#selected_client_agent').val());
			data.append('status_id',$('#selected_profile_client_status').val());
			data.append('case_classification_id',$('#selected_case_classification').val());
...
function populate_profile_data(client_date){
	var data = new FormData();
...
function create_or_update_covid_client_screening(tmp_client_id,client_id,questionnaire_id,score){
	var data = new FormData();
	data.append('create_or_update_covid_client_screening',1);
	data.append('client_id',client_id);
	data.append('prep_area',general_variable_0);
	data.append('tmp_client_id',tmp_client_id);
	data.append('questionnaire_id',questionnaire_id);
...
function fetch_prep_ussd(){	
	var data = new FormData();
	data.append('fetch_prep_ussd',1);
...
function fetch_sms_ussd(){	
	var data = new FormData();
	data.append('fetch_sms_ussd',1);
...
function fetch_prep_report(display_type){
	var data = new FormData();
	
	data.append('fetch_prep_report',1);
	data.append('display_type',display_type);
	data.append('rule_string',$('#report_rule_string').val());
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function create_new_prep_report(report_id){
	show_window('item_details');
	change_window_size('item_details',820,500,1);
	
	$('#item_details_title').html('Custom Report');
	
	show_loading_progress('item_details_holder','Preparing options. Wait...');
	
	var data = new FormData();
	data.append('create_new_prep_report',1);
	data.append('module_id',$('#module_id').val());
	data.append('report_id',report_id);
	data.append('screening_question_level_rule_string',$('#screening_question_level_rule_string').val());
	data.append('form_category_level_rule_string',$('#form_category_level_rule_string').val());
	data.append('form_rule_string',$('#form_rule_string').val());
	data.append('column_rule_string',$('#column_string').val());
	data.append('column_value_string',$('#column_value_string').val());
	data.append('profile_rule_string',$('#profile_rule_string').val());
	data.append('screening_rule_string',$('#screening_rule_string').val());
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function create_new_ussd_report(report_id){
	show_window('item_details_1');
	change_window_size('item_details_1',820,500,1);
	$('#item_details_title_1').html('Custom Report');
	
	show_loading_progress('item_details_holder_1','Preparing options. Wait...');
	
	var data = new FormData();
	data.append('create_new_ussd_report',1);
	data.append('report_id',report_id);
	data.append('report_rule_string',$('#report_rule_string').val());
	process_simultanious_xmlhttp('module_xmlhttp',data);
}
...
function change_client_status(client_id,new_status,client_date){
...
function delete_client(client_id,client_date){
...
function save_prep_custom_report(report_id){
	if($('#screening_rule_string').val() == '' && $('#profile_rule_string').val() == '' && $('#form_rule_string').val() == ''){
		alert('You need to select at least one field to include on your report');
		
	}else{
		show_window('item_details_1',1);
		$('#item_details_title_bar_1').css('background-color','orange');
		$('#item_details_title_1').html('Save Custom Report');
		
		show_loading_progress('item_details_holder_1','Preparing options. Wait...');
		
		var data = new FormData();
		data.append('save_prep_custom_report',1);
		data.append('report_id',report_id);
...
function process_save_prep_report_2(){
	if($('#save_prep_report').html() != 'Wait...'){
		
		if($('#prep_report_title').val() == 'Enter title here' || $('#prep_report_title').val() == ''){
			alert('You need to provide the title for this report');
			
		}else{
		
			var c = confirm('Are you sure you wish to save this report?');
			
			if(c){
				var data = new FormData();
				data.append('process_save_prep_report_2',1);
				data.append('report_id',$('#selected_quick_report').val());
				data.append('title',$('#prep_report_title').val());
				data.append('form_rule_string',$('#form_rule_string').val());
				data.append('screening_question_level_rule_string',$('#screening_question_level_rule_string').val());
				data.append('form_category_level_rule_string',$('#form_category_level_rule_string').val());
				data.append('column_string',$('#column_string').val());
				data.append('option_string',$('#column_value_string').val());
				data.append('screening_rule_string',$('#screening_rule_string').val());
				data.append('profile_rule_string',$('#profile_rule_string').val());
				...
				data.append('site_id',$('#selected_user_site').val());
				data.append('branch',$('#selected_user_cluster').val());
				data.append('group_ids',$('#selected_group_ids').val());
				data.append('unit_id',$('#selected_user_unit').val());
				data.append('report_default',$('#selected_default_report').val());
				data.append('accessibility_type',$('#selected_user_accessibility').val());
				
				$('#save_prep_report').html('Wait...');
...
function delete_prep_report(){
	if($('#delete_custom_report_button').html() != 'Wait...'){
		var c = confirm('Are you sure you wish to delete this report?');

		if(c){
			var data = new FormData();
			data.append('delete_prep_report',1);
			data.append('report_id',$('#selected_quick_report').val());
			
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#delete_custom_report_button').html('Wait...');
		}
	}
}

function process_prep_report(){
	//if($('#process_prep_report_button').html() != 'Wait...'){
		
		var data = new FormData();
		data.append('process_prep_report',1);
		...
		data.append('column_string',$('#column_string').val());
		data.append('column_option_string',$('#column_value_string').val());
		
		data.append('retrieval_type',$('#selected_data_retrieval').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		$('#process_prep_report_button').html('Wait...');		
	//}
}
...
function fetch_dynamic_form_list(form_id,ignore_form_data,client_date){
	$('#client_profile').hide();
...
function fetch_dynamic_form_details(form_id,data_set_id,client_date){
	$('#client_profile').hide();
	$('#client_screening').hide();
	$('#client_update_holder').slideDown('fast');
...
function create_or_update_custom_client_form(form_id,data_set_id,loading_style,client_date){
	if($('#dynamic_form_save_button_'+form_id).html() != 'Wait...'){
		$('#custom_form_error_message_'+form_id).slideUp('fast');
		$('#custom_form_error_message_'+form_id).html('');
		form_validation_successful = 0;
		if(form_id == 2){
			
			if($('#selected_locator_day').val() == 0 || $('#selected_locator_month').val() == 0 || $('#selected_locator_year').val() == 0){
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Please enter the date appearing on the hard copy');
				$('#locator_date_holder').css('border','solid 1px red');
				
			}else if($('#locator_date_validation').val() == 0){
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Investigation date is in the future. Please check the date provided');
				$('#locator_date_holder').css('border','solid 1px red');
			
			}else if($('#option_109').val() == '' && $('#option_110').val() == '' && $('#option_111').val() == ''){
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('You need to select the entry type');
				$('#entry_type_holder').css('border-color','red');
				
			}else if($('#option_110').val() == 1 && $('#option_110_text').val() == 'Enter facility here'){
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Please enter facility where this client is being transfered from');
				$('#entry_type_holder').css('border-color','red');
			...
			/*	
				...
				*/
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter the highest grade for this client');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter home address for this client');
				$('#education_level_holder').css('color','#000');
				$('#option_119_text').css('border-color','red');
				
			}else if($('#option_120_text').val() == 'Enter street name here'){
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter street name for this client');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter township and town for this client');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter emergency contact name');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter relationship with provided emergency contact');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter house number for emerrgency contact');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter street name for provided contact');
				...
				$('#custom_form_error_message_2').slideDown('fast');
				$('#custom_form_error_message_2').html('Enter town and township for provided emergency contact');
			...
			if($('#selected_initiation_day').val() == 0 || $('#selected_initiation_month').val() == 0 || $('#selected_initiation_year').val() == 0){
				$('#custom_form_error_message_3').slideDown('fast');
				$('#custom_form_error_message_3').html('Please enter the investigation date as it appears on the hard copy');
				$('#initiation_date_holder').css('border','solid 1px red');
				
				level_1_validation = 0;
				
			}else if($('#initiation_date_validation').val() == 0){
				$('#custom_form_error_message_3').slideDown('fast');
				$('#custom_form_error_message_3').html('Initiation date cannot be in the future. Please correct intiation date');
				$('#initiation_date_holder').css('border','solid 1px red');
				
				level_1_validation = 0;	
				
			}else if($('#clinical_date_validation').val() == 0 || $('#pharmacy_date_validation').val() == 0){
				$('#custom_form_error_message_3').slideDown('fast');
				$('#custom_form_error_message_3').html('Please scroll down and check next appointment dates.');
				...
				if($('#option_357').val() == 1){
					if($('#option_359').val() == '' && $('#option_360').val() == ''){
						$('#custom_form_error_message_3').slideDown('fast');
						$('#custom_form_error_message_3').html('You need to select how many people client had sex with');
						...
						$('#custom_form_error_message_3').slideDown('fast');
						$('#custom_form_error_message_3').html('You need to accept at-least one risky behaviour engaged in by client\'s sex partner');
			...
			if(level_1_validation){
				if(($('#option_372').val() == '' || $('#option_373').val() == 1) && ($('#option_374').val() ==1 || $('#option_376').val() == 1)){
					...
					$('#custom_form_error_message_3').slideDown('fast');
					$('#custom_form_error_message_3').html('Check options selected on client\'s partner\'s HIV treatment');
			...
			if($('#client_selected_sex').val() == 2){
				if($('#option_136').val() == -1){
					$('#custom_form_error_message_3').slideDown('fast');
					$('#custom_form_error_message_3').html('Please specify if the client is pregnant');
				
					$('#partner_hiv_treatment').css('color','red');
					form_validation_successful = 0;
				
				}else if($('#option_139').val() == -1){
					$('#custom_form_error_message_3').slideDown('fast');
					$('#custom_form_error_message_3').html('Please indicate if client is breastfeeding or not');
			...
			if($('#is_dreams_site').val() == 1){
				if($('#option_765').val() == -1){
					$('#custom_form_error_message_3').slideDown('fast');
					$('#custom_form_error_message_3').html('Please indicate if dreams intervention was offered');
			...
			if($('#selected_follow_up_day').val() == 0 || $('#selected_follow_up_month').val() == 0 || $('#selected_follow_up_year').val() == 0){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('Please enter the investigation date as it appears on the hard copy');
				$('#follow_up_date_holder').css('border','solid 1px red');
				
			
			}else if($('#follow_up_date_validated').val() == 0){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('There is another follow up form with the investigation date provided. Please check the date');
				$('#follow_up_date_holder').css('border','solid 1px red');
				
			}else if($('#follow_up_date_validated').val() == 2){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('Provided investigation date is earlier than initiation form for this client.');
				$('#follow_up_date_holder').css('border','solid 1px red');
				
			}else if($('#follow_up_date_validated').val() == 3){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('Provided investigation date is in the future. Please check the date');
				$('#follow_up_date_holder').css('border','solid 1px red');
				
			}else if($('#option_768_text').val() == ''){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('You need to select the population category for this client');
				
			}else if($('#follow_up_clinical_date_validation').val() == 0 || $('#follow_up_pharmacy_date_validation').val() == 0){
				$('#custom_form_error_message_1').slideDown('fast');
				$('#custom_form_error_message_1').html('Scroll down and check the next appointment dates');
			...
			if($('#selected_adherence_day').val() == 0 || $('#selected_adherence_month').val() == 0 || $('#selected_adherence_year').val() == 0){
				$('#custom_form_error_message_6').slideDown('fast');
				$('#custom_form_error_message_6').html('Please enter the investigation date as it appears on the hard copy');
				$('#adherence_date_holder').css('border','solid 1px red');
				
			}else if($('#adherence_date_validation').val() == 0){
				$('#custom_form_error_message_6').slideDown('fast');
				$('#custom_form_error_message_6').html('Date of investigation cannot be in the future. Please correct the date');
				$('#adherence_date_holder').css('border','solid 1px red');
		...
		if(form_validation_successful){
			if(loading_style == undefined || loading_style == 0){
...
function delete_prep_form(form_id,data_set_id,client_date){
...
function fetch_questionnaire_ession(session_index,question_index){
	...
	var session_div = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#question_dependency_ession_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_ession_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#active_question_dependency_question_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#active_question_dependency_option_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_ession_'+session_index+'_'+question_index+'\').val(-1);$(\'#selected_question_dependency_question_'+session_index+'_'+question_index+'\').val(-1);$(\'#selected_question_dependency_option_'+session_index+'_'+question_index+'\').val(-1);">None</div>';
	
	for(var s=0;s<total_sessions;s++){
		//if(s != session_index){
			if(session_div == ''){
				session_div = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#question_dependency_ession_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_ession_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_ession_'+session_index+'_'+question_index+'\').val('+s+');fetch_question_advanced_option_questions('+session_index+','+question_index+');">Session '+(s+1)+'</div>';
...
function fetch_question_advanced_option_questions(session_index,question_index){
	var source_session_index = $('#selected_question_dependency_ession_'+session_index+'_'+question_index).val();
	
	var total_questions = Number($('#total_questions_'+source_session_index).val());
	
	var question_string = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_question_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_question_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#active_question_dependency_option_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_question_'+session_index+'_'+question_index+'\').val(-1);$(\'#selected_option_question_dependency_option_'+session_index+'_'+question_index+'\').val(-1);">None</div>';
	for(var q=0;q<total_questions;q++){
		if(source_session_index != session_index || (source_session_index == session_index && q!= question_index )){
			if(question_string == ''){
				question_string = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_question_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_question_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_question_'+session_index+'_'+question_index+'\').val('+q+');fetch_question_advanced_option_options('+session_index+','+question_index+');">Question '+(q+1)+'</div>';
				
			}else{
				question_string = question_string+'<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_question_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_question_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_question_'+session_index+'_'+question_index+'\').val('+q+');fetch_question_advanced_option_options('+session_index+','+question_index+');">Question '+(q+1)+'</div>';
				
			}
		}
	}
	
	$('#option_question_dependency_question_menu_'+session_index+'_'+question_index).html(question_string);
}


function fetch_question_advanced_option_options(session_index,question_index){
	var source_session_index = $('#selected_question_dependency_ession_'+session_index+'_'+question_index).val();
	var source_question_index = $('#selected_question_dependency_question_'+session_index+'_'+question_index).val();
	
	var total_options = Number($('#total_question_options_'+source_session_index+'_'+source_question_index).val());
	
	var option_string = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_option_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_option_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_option_'+session_index+'_'+question_index+'\').val(-1);">None</div>';
	for(var o=0;o<total_options;o++){		
		if(option_string == ''){
			option_string = '<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_option_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_option_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_option_'+session_index+'_'+question_index+'\').val('+o+');">Option '+(o+1)+'</div>';
			
		}else{
			option_string = option_string+'<div class="option_menu_item" onmouseover="this.style.color=\'#006bb3\';" onmouseout="this.style.color=\'#000\';" onclick="$(\'#option_question_dependency_option_menu_'+session_index+'_'+question_index+'\').toggle(\'fast\');$(\'#active_question_dependency_option_'+session_index+'_'+question_index+'\').html($(this).html());$(\'#selected_question_dependency_option_'+session_index+'_'+question_index+'\').val('+o+');">Option '+(o+1)+'</div>';
			
		}
	}
	
	$('#option_question_dependency_option_menu_'+session_index+'_'+question_index).html(option_string);
}
...
function fetch_sms_client_details(client_id){
	...
	show_loading_progress('item_details_'+new_window_index+'_holder','Preparing data. Wait...');
...
function import_multiple_clients(){
	var new_window_index = create_data_window();
	change_window_size('item_details_'+new_window_index,600,500,1);
	show_window('item_details_'+new_window_index);
	
	$('#item_details_holder_'+new_window_index).css('height','450px');
	$('#item_details_title_'+new_window_index).html('Import Multiple Clients');
	...
	var data = new FormData();
	data.append('import_multiple_clients',1);
	data.append('index_id',new_window_index);
...
function export_survey_report(){
	if($('#client_export_button').html() != 'Wait...'){
		if($('#selected_analytical_survey_id').val() == 0){
			alert('You need to select a survey first');
			
		}else{
			var c = confirm('Are you sure you wish to proceed with exporting this survey?');
			
			if(c){			
				var data = new FormData();
				data.append('export_survey_report',1);
				data.append('survey_id',$('#selected_analytical_survey_id').val());
				data.append('status',$('#selected_analytical_survey_status').val());
...
function fetch_error_reports(){
	...
	$('#item_details_title_bar_1').css('background-color','brown');
	$('#item_details_title_1').html('Fetch Entries with Errors');
	
	show_loading_progress('item_details_holder_1','Preparing options. Wait...');
	
	var data = new FormData();
	data.append('fetch_error_report',1);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
}

function process_error_report(){
	//if($('#report_error_report_button').html() != 'Wait...'){
		var error_type_ids = $('#error_type_ids').val();
		
		
		
		var data = new FormData();
		data.append('process_error_report',1);
		data.append('error_type_ids',error_type_ids);
		data.append('from_date',$('#date_from_report').val());
		data.append('to_date',$('#date_to_report').val());
		...
		data.append('presentation',$('#selected_presentation').val());
		
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#report_error_report_button').html('Wait...');
	//}
}

function fetch_missed_appointments(){	
	var data = new FormData();
	data.append('fetch_error_reports',1);
	//data.append('client_status',client_status);
...
function validate_follow_up_date(data_set_id,form_id,client_date,option_id){
	if($('#selected_follow_up_day').val() != 0 && $('#selected_follow_up_month').val() != 0 && $('#selected_follow_up_year').val() != 0){
		
		var data = new FormData();
		data.append('validate_follow_up_date',1);
		data.append('client_id',$('#client_id').val());
		data.append('form_id',form_id);
		data.append('option_id',option_id);
		data.append('option_value',$('#option_'+option_id).val());
		data.append('data_set_id',data_set_id);
		data.append('client_date',client_date);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
	
		$('#date_validation_status').html('Checking if form with similar date exit...');
	
		$('#follow_up_date_validated').val(0);
	}
}

function is_date_future(date_input,today_date){
	if(today_date == 0){
		var today = new Date();
	...
	var future_date = new Date(date_input);
	
	var date_difference = (future_date - today)/1000;
	...
	output_array[1] = date_difference;
	
	return output_array;
}

function send_offline_pipat(){
	var data = new FormData();
	data.append('send_offline_pipat',1);
	
	$('#send_offline_pipat_button').html('Wait...');
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_covid_client_list(tab_id){
	var data = new FormData();
	
	data.append('fetch_covid_client_list',1);
	data.append('tab_id',tab_id);
	data.append('branch_id',$('#selected_branch').val());
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province_id').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('mother_facility_id',$('#selected_mother_facility').val());
	data.append('site_id',$('#selected_status').val());
	data.append('sreening_method',$('#selected_screening_method').val());
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	show_loading_progress('clients','Fetching data. Wait...');
}

function fetch_covid_whatsapp(){
	var data = new FormData();
	data.append('fetch_covid_whatsapp',1);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_covid_ussd(){
	var data = new FormData();
	data.append('fetch_covid_ussd',1);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_covid_questionnaire_list(){
	var data = new FormData();
	data.append('fetch_covid_questionnaire_list',1);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_stories_list(story_status){
	var data = new FormData();
	
	data.append('fetch_stories_list',1);
	data.append('story_status',story_status);
	data.append('this_branch_id',$('#selected_branch').val());
	data.append('unit_id',$('#selected_unit').val());
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	...
	show_loading_progress('stories','Fetching data. Wait...');
	
}

function update_agent_type(agent_id,new_status){
	if($('#finance_processign_button').html() != 'Wait...'){
		var c = confirm('Are you sure you wish to make this update?');
		
		if(c){
			var data = new FormData();
			data.append('update_agent_type',1);
			data.append('agent_id',agent_id);
			data.append('new_status',new_status);
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#finance_processign_button').html('Wait...');
...
function fetch_partition_details(partition_id){	
	var data = new FormData();
	data.append('fetch_partition_details',1);
	data.append('partition_id',partition_id);
...
function fetch_tempreture_scanner(){
	var data = new FormData();
	
	data.append('fetch_tempreture_scanner',1);
	data.append('tab_id',1);
	data.append('branch_id',$('#selected_branch').val());
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province_id').val());
	data.append('hub_id',$('#selected_hub').val());
...
function export_tempreture_scanner(){
	var data = new FormData();
	
	data.append('export_tempreture_scanner',1);
	data.append('branch_id',$('#selected_branch').val());
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province_id').val());
	data.append('hub_id',$('#selected_hub').val());
	...
	$('#tempreture_export_button').html('Wait...');
}
...
function update_system_settings(){
	...
	if(c){
		var data = new FormData();
		data.append('update_system_settings',1);
		
		data.append('limit_attempts',$('#selected_limit_login_attempts').val());
		data.append('allowed_attempts',$('#allowed_attempts').val());
		data.append('action_on_limit',$('#selected_action_on_limit_login_attempts').val());
		
		data.append('reattempt_delay',$('#selected_attempts_delay').val());
		data.append('delay_interval',$('#delay_interval').val());
		
		data.append('password_reset',$('#selected_password_reset').val());
		data.append('password_strict',$('#selected_password_strict').val());
		data.append('password_length',$('#password_length').val());
		
		data.append('password_expiry',$('#selected_password_expiry').val());
		data.append('password_expiry_period',$('#password_expiry_period').val());
		
		data.append('auto_backup',$('#selected_auto_backup').val());
		data.append('backup_interval',$('#selected_backup_interval').val());
		data.append('backup_time',$('#selected_from_hour').val()+'/'+$('#selected_from_minute').val()+'/'+$('#selected_from_second').val());
		data.append('username_length',$('#username_length').val());
		
		data.append('url_password_reset',$('#selected_url_password_reset').val());
		data.append('url_password_expiry_period',$('#url_password_reset_period').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		$('#update_or_create_settings_button').html('Wait...');
	}	
}


function check_password_and_username(this_user_id){
	var data = new FormData();
	data.append('check_password_and_username',1);
	data.append('this_user_id',this_user_id);
	data.append('username',$('#user_username').val());
	data.append('password',$('#user_password').val());
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#login_check_status').slideUp('fast');
	$('#login_check_status').html('<div style="width:100%;height:20px;line-height:20px;text-align:center;">Checking login credentials validity</div>');	
}

function allocate_agent(){
	if($('#selected_agent_validations').val() == ''){
		alert('You need to select atleast 1 agent');
		
	}else{
		show_window('item_details',1);
		
		change_window_size('item_details',400,300,1);
		
		$('#item_details_title').html('Allocate agent to user');
		
		var data = new FormData();
		data.append('allocate_agent',1);
		data.append('select_agents',$('#selected_agent_validations').val());
...
function search_allocation_users(_type){
	var data = new FormData();
	data.append('search_allocation_users',1);
	
	if(!_type){
		$('#agent_search_result_holder').slideDown('fast');
		data.append('search_key',$('#allocation_users_search_key').val());
		
		$('#search_results_holder').html('<div style="width:100%;height:25px;line-height:25px;float:left;text-align:center;margin-top:20px;">Searching users. Wait...</div>');
	
	}else{
		$('#agent_search_result_holder1').slideDown('fast');
		data.append('search_key',$('#allocation_users_search_key1').val());
...
function complete_allocate_user(){
	if($('#user_allocation_button').html() != 'Wait..'){
		var c = confirm('Are you sure you wish to allocate selected claims to this user');
		
		if(c){
			var data = new FormData();
			data.append('complete_allocate_user',1);
			data.append('selected_agents',$('#selected_agent_validations').val());
			data.append('user_allocation',$('#selected_allocation1').val());
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#user_allocation_button').html('Wait...');
		}
...
function update_or_create_application(app_id){
	if($('#app_name').val() == '' || $('#app_name').val() == 'Enter app name here'){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter application name');
		
	}else if($('#app_organisation').val() == '' || $('#app_organisation').val() == 'Enter app organisation here'){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter organisation to which this application is assigned');
		
	}else if($('#app_password').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('You need to provide the app password');
		...
		if(c){
			var data = new FormData();
			data.append('update_or_create_application',1);
			data.append('app_id',app_id);
			data.append('application_name',$('#app_name').val());
			data.append('organisation',$('#app_organisation').val());
			data.append('app_password',$('#app_password').val());
			data.append('app_branch_id',$('#selected_app_cluster').val());
			data.append('allowed_module',$('#allowed_module').val());
			data.append('selected_app_action',$('#selected_app_action').val());
			data.append('organisational_key',$('#organisational_key').val());
			data.append('change_password_id',$('#change_password_id').val());
			data.append('app_linkage',$('#selected_app_linkage').val());
			
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#update_or_create_app_button').html('Wait...');
			
		}
	}
}

function change_app_status(new_status,app_id){
	var c = confirm('Are you sure you wish to continue?');
	
	if(c){
		var data = new FormData();
		data.append('change_app_status',1);
		data.append('new_status',new_status);
		data.append('app_id',app_id);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#disable_app_button').html('Wait...');
	}
}

function send_password_reset_link(user_id){
	var c= confirm('Are you sure you wish to proceed?');
	
	if(c){
		var data = new FormData();
		data.append('send_password_reset_link',1);
		data.append('user_id',user_id);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#password_reset_link_button').html('Wait...');
	}
}

function check_partition_dates(query_input){
	var date_from = $('#partition_from').val();
	var date_to = $('#partition_to').val();
	
	var date_from_array = date_from.split('/');
	var date_to_array = date_to.split('/');
	
	var from_date = new Date(date_from_array[2],date_from_array[0],date_from_array[1],0,0,0);
	var to_date = new Date(date_to_array[2],date_to_array[0],date_to_array[1],23,59,59);
	...
	if(!query_input){
		if(from_date_time > to_date_time){
			$('#partition_to').val($('#partition_from').val());
		}
		
	}else{
		if(from_date_time > to_date_time){
			$('#partition_from').val($('#partition_to').val());
		}
	}
}

function check_other_partition_date(partition_type,partition_id){
	var data = new FormData();
	data.append('check_other_partition_date',1);
	data.append('partition_from',$('#partition_from').val());
	data.append('partition_to',$('#partition_to').val());
	data.append('partition_type',partition_type);
	data.append('partition_id',partition_id);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#period_from_validation_status').html('Checking date validity...');
	$('#period_from_validation').val(0);
}

function update_or_create_partition(partition_type,partition_id){
	if($('#partition_from').val() == '' || $('#period_from_validity').val() == 0){
		alert('Period from cannot be empty and must pass validation');
		...
		if(c){
			var data = new FormData();
			data.append('update_or_create_partition',1);
			data.append('partition_type',partition_type);
			data.append('partition_id',partition_id);
			data.append('partition_from',$('#partition_from').val());
			data.append('partition_to',$('#partition_to').val());
			...
			$('#create_or_update_partition_button').html('Wait...');
		}
	}
}

function delete_partition(partition_type,partition_id){
	var c = confirm('Are you sure you wish to proceed?');
	
	if(c){
		var data = new FormData();
		data.append('delete_partition',1);
		data.append('partition_type',partition_type);
		data.append('partition_id',partition_id);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
			
		$('#delete_partition_button').html('Wait...');
	}
}

function fetch_activity_calendar_list(activity_status){
	var data = new FormData();
	data.append('fetch_activity_calendar_list',1);
	data.append('activity_status',activity_status);
	data.append('branch_id',$('#selected_branch').val());
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province_id').val());
	data.append('hub_id',$('#selected_hub').val());
	...
	show_loading_progress('activity_calendar_holder','Fetching data. Wait...');
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_calendar_activity(activity_id){
	show_window('item_details',1);
	
	show_loading_progress('item_details_holder','fetching data. Wait...');
	
	var data = new FormData();
	data.append('fetch_calendar_activity',1);
	data.append('activity_id',activity_id);
	process_simultanious_xmlhttp('module_xmlhttp',data);
	change_window_size('item_details',800,400,1);
	$('#item_details_title').html('Activity details');
}

function create_or_update_activity(activity_id){
	if($('#activity_subject').val() == 'Enter subject here' || $('#activity_subject').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter activity subject');
		
	}else if($('#activity_objective').val() == 'Enter activity objective here' || $('#activity_objective').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter activity objective');
		
	}else if($('#activity_budget_code').val() == 'Enter activity budget code here'|| $('#activity_budget_code').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter budget code');
		
	}else if($('#activity_participants').val() == 0 || $('#activity_participants').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Enter total participants for this activity');
		
	}else if($('#selected_activity_hub').val() == 0){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Hub cannot be empty');
		
	}else{
		var c = confirm('Are you sure you wish to progress?');
		if(c){
			var data = new FormData();
			data.append('create_or_update_activity',1);
			data.append('activity_id',activity_id);
			data.append('title',$('#activity_subject').val());
			data.append('activity_objective',$('#activity_objective').val());
			data.append('activity_budget_code',$('#activity_budget_code').val());
			data.append('activity_date_from',$('#activity_date_from').val());
			data.append('activity_date_to',$('#activity_date_to').val());
			data.append('activity_participants',$('#activity_participants').val());
			data.append('unit_id',$('#selected_activity_unit').val());
			data.append('region_id',$('#selected_activity_region').val());
			data.append('province_id',$('#selected_activity_province').val());
			data.append('hub_id',$('#selected_activity_hub').val());
			data.append('site_id',$('#selected_activity_site').val());
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#create_or_update_activity_button').html('Wait...');
		}
	}
}

function check_activity_active(activity_id){
	if($('#activity_active_'+activity_id).val() == 0){
		$('#activity_header_'+activity_id).css('background-color','#bbd7dd'),
		$('#activity_header_'+activity_id).attr('onmouseover',"this.style.backgroundColor='#a9ccd4'"),
		$('#activity_header_'+activity_id).attr('onmouseout',"this.style.backgroundColor='#bbd7dd'"),
		
		$('#activity_active_'+activity_id).val(1);
		
	}else{
		$('#activity_header_'+activity_id).css('background-color',''),
		$('#activity_header_'+activity_id).attr('onmouseover',"this.style.backgroundColor='#eee'"),
		$('#activity_header_'+activity_id).attr('onmouseout',"this.style.backgroundColor=''"),
		
		$('#activity_active_'+activity_id).val(0);
	}
}

function add_activity_budget(activity_id){
	show_window('item_details',1);
	
	change_window_size('item_details',400,400,1);
	$('#item_details_title').html('Add budget to activity');
	
	show_loading_progress('item_details_holder','Preparing. Wait...');
	
	var data = new FormData();
	data.append('add_activity_budget',1);
	data.append('activity_id',activity_id);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function add_file_to_budget(){
	if($('#uploaded_files').val() == ''){
		alert('You need to add at-least one file');
	
	}else{
		var original_div_holder = '<div style="width:100%;height:90px;line-height:90px;text-align:center;float:left;margin-top:5px;background-color:#eee;font-size:1.3em;cursor:pointer;" onmouseover="this.style.backgroundColor=\'#ddd\'" onmouseout="this.style.backgroundColor=\'#eee\';" title="Click to upload file" onclick="open_uploader(\'add_file_to_budget()\',0)">Click to add budget file</div>';
		
		var div_code = '<div style="color:#fff;cursor:pointer;padding:3px;max-width:350px;width:auto;min-height:30px;height:auto;line-height:30px;float:left;background-color:#9a6f9a" onmouseover="this.style.backgroundColor=\'#8f618f\'" onmouseout="this.style.backgroundColor=\'#9a6f9a\'"><div style="width:auto;height:auto;float:left;">'+$('#uploaded_files').val()+'</div><div style="background-color:brown;width:20px;height:30px;float:right;color:#fff;line-height:30px;text-align:center;margin-left:5px;" onclick="remove_activity_upload();">X</div></div>';
		
		$('#activity_budget_file').val($('#uploaded_files').val());
		
		$('#activity_budget_file_holder').html(div_code);
		
		reset_image_upload();
		
		$('#image_uploader').fadeOut('fast');
		
	}
}

function remove_activity_upload(){
	var c = confirm('Are you sure you wish to remove this file?');
	
	if(c){
		var original_div_holder = '<div style="width:100%;height:90px;line-height:90px;text-align:center;float:left;margin-top:5px;background-color:#eee;font-size:1.3em;cursor:pointer;" onmouseover="this.style.backgroundColor=\'#ddd\'" onmouseout="this.style.backgroundColor=\'#eee\';" title="Click to upload file" onclick="open_uploader(\'add_file_to_budget()\',0)">Click to add budget file</div>';
	
		$('#activity_budget_file').val('');
		$('#activity_budget_file_holder').html(original_div_holder);
	}
}

function process_add_activity_budget(activity_id){
	if($('#add_activity_budget_button').html() != 'Wait...'){
		if($('#budget_number').val() == '' || $('#budget_number').val() == 'Enter budget number here'){
			$('#error_message').slideDown('fast');
			$('#error_message').html('You need to provide the budget number');
			
		}else if($('#claim_value').val() == '' || $('#claim_value').val() == 0){
			$('#error_message').slideDown('fast');
			$('#error_message').html('You need to provide the claim value');
			
		}else if($('#activity_budget_file').val() == ''){
			$('#error_message').slideDown('fast');
			$('#error_message').html('Please attach budget file');
			...
			if(c){
				var data = new FormData();
				data.append('process_add_activity_budget',1);
				data.append('budget_number',$('#budget_number').val());
				data.append('activity_id',activity_id);
				data.append('create_claim',$('#enable_claims').val());
				data.append('beneficiaries',$('#claim_beneficiaries').val());
				data.append('claim_value',$('#claim_value').val());
				data.append('payment_from',$('#claim_date_from').val());
				data.append('payment_to',$('#claim_date_to').val());
				data.append('budget_file',$('#activity_budget_file').val());
				
				process_simultanious_xmlhttp('module_xmlhttp',data);
				$('#add_activity_budget_button').html('Wait...');
...
function reorganise_data_sets(client_id,client_date,last_data_set_date,form_id){
	var data = new FormData();
	data.append('reorganise_data_sets',1);
	data.append('client_id',client_id);
	data.append('client_date',client_date);
	data.append('last_data_set_date',last_data_set_date);
	data.append('form_id',form_id);
...
function fetch_meetings(){
	var data = new FormData();
	data.append('fetch_meetings',1);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
	data.append('search_key_string',$('#search_key_string').val());
	
	show_loading_progress('meeting_holder','Preparing. Wait...');
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
}

function fetch_meeting_batches(){
	var data = new FormData();
	data.append('fetch_meeting_batches',1);
	data.append('date_from',$('#date_from').val());
	data.append('date_to',$('#date_to').val());
	data.append('search_key_string',$('#search_key_string').val());
	data.append('region_id',$('#selected_region').val());
	...
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
	data.append('with_files',$('#selected_with_files').val());
	data.append('status',$('#selected_status').val());
...
function fetch_meeting_participants(){
	var data = new FormData();
	data.append('fetch_meeting_participants',1);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
...
function move_participants(meeting_id){
	if($('#selected_participants_'+meeting_id).val() == ''){
		alert('You need to select atleast one participant');
		
	}else{
		show_window('item_details',1);
		show_loading_progress('item_details_holder','Loaidng. Wait...');
		$('#item_details_title').html('Move participants');
		
		change_window_size('item_details',400,400,1);
		
		var data = new FormData();
		data.append('move_participants',1);
...
function check_meeting_code(){
	var new_meeting_id = $('#new_meeting_id').val();
	
	if(new_meeting_id.includes(' ')){
		alert('New meeting ID cannot have a space in it. Correct new meeting ID and try again');
		
	}else{
		var data = new FormData();
		data.append('check_meeting_code',1);
		data.append('new_meeting_id',$('#new_meeting_id').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#move_meeting_now_holder').slideUp('fast');
		$('#moving_to_holder').slideUp('fast');
		
		$('#check_meeting_holder').slideDown('fast');
		$('#meeting_check_holder_details').html('<div style="width:100%;height:20px;line-height:20px;text-align:center;margin-top:10px;">Checking meeting code. Wait....</div>');
		
		$('#meeting_code_check_button').html('Wait...');
	}
	
}

function confirm_move_meeting_participants(meeting_id){
	var new_meeting_id = $('#new_meeting_id').val();
	if($('#selected_participants_'+meeting_id).val() == ''){
		alert('You need to select atleast one participant');
		
	}else if(new_meeting_id.includes(' ')){
		alert('New meeting ID cannot have a space in it. Correct new meeting ID and try again');
		...
		if(c){
			var data = new FormData();
			data.append('confirm_move_meeting_participants',1);
			data.append('meeting_id',meeting_id);
			data.append('new_meeting_id',$('#new_meeting_id_input').val());
			data.append('selected_participants',$('#selected_participants_'+meeting_id).val());

			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#move_participants_meeting_button').html('Wait...');
		}
	}
}

function view_meeting_details(meeting_id,parent_id){
	show_window('item_details',1);
	show_loading_progress('item_details_holder','Loaidng. Wait...');
	$('#item_details_title').html('Meeting details');
	
	var data = new FormData();
	data.append('view_meeting_details',1);
	data.append('meeting_id',meeting_id);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
...
function create_or_update_meeting(meeting_id){
	
	if($('#meeting_title').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('You need to enter the meeting title');
		
	}else if($('#meeting_from').val() == '' || $('#meeting_to').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('Meeting validity dates cannot be empty');
		
		
	}else if($('#selected_meeting_code').val() == 1 && $('#meeting_code').val() == ''){
		$('#error_message').slideDown('fast');
		$('#error_message').html('You need to enter meeting code');
		...
		if(c){
			$('#error_message').slideUp('fast');
			var data = new FormData();
			data.append('create_or_update_meeting',1);
			data.append('meeting_id',meeting_id);
			data.append('title',$('#meeting_title').val());
			data.append('meeting_password',$('#meeting_password').val());
			data.append('meeting_from',$('#meeting_from').val());
			data.append('meeting_to',$('#meeting_to').val());
			data.append('meeting_code_type',$('#selected_meeting_code').val());
			data.append('meeting_code',$('#meeting_code').val());
			data.append('region_id',$('#selected_meeting_region').val());
			data.append('province_id',$('#selected_meeting_province').val());
			data.append('hub_id',$('#selected_meeting_hub').val());
			data.append('site_id',$('#selected_meeting_site').val());
			data.append('unit_id',$('#selected_meeting_unit').val());
			data.append('activity_id',$('#selected_meeting_activity').val());
			data.append('meeting_parent_id',$('#meeting_parent_id').val());
			data.append('meeting_teams',$('#selected_meeting_team').val());
			data.append('number_of_teams',$('#number_of_meeting_teams').val());
			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#create_or_update_meeting_button').html('Wait...');
		}
		
	}
}

function expand_collapse_meeting(meeting_id){
	if($('#meeting_expanded_'+meeting_id).val() == 0){
		$('#meeting_title_'+meeting_id).css('background-color','#76a5af');
		$('#meeting_title_'+meeting_id).css('color','#fff');
		$('#meeting_title_'+meeting_id).attr("onmouseover","this.style.backgroundColor='#668c94'");
		$('#meeting_title_'+meeting_id).attr("onmouseout","this.style.backgroundColor='#76a5af'");
		
		$('#meeting_details_holder_'+meeting_id).slideDown('fast');
		$('#meeting_expanded_'+meeting_id).val(1);
		
		$('#meeting_details_'+meeting_id).html('<div style="width:100%;height:30px;text-align:center;line-height:30px;font-size:1.2em;margin-top:30px;">Loading participants. Wait...</div>');
		
		var data = new FormData();
		data.append('show_meeting_participants',1);
		data.append('meeting_id',meeting_id);
		
		process_simultanious_xmlhttp('meeting_xmlhttp_'+meeting_id,data);
		
	}else{
		if($('#meeting_batched_'+meeting_id).val() == 1){
			$('#meeting_title_'+meeting_id).css('background-color','#c2eae9');
			$('#meeting_title_'+meeting_id).css('color','#000');
			
			$('#meeting_title_'+meeting_id).attr("onmouseover","this.style.backgroundColor='#b2dad9'");
			$('#meeting_title_'+meeting_id).attr("onmouseout","this.style.backgroundColor='#c2eae9'");
			
		}else{
			$('#meeting_title_'+meeting_id).css('background-color','');
			$('#meeting_title_'+meeting_id).css('color','#000');
			
			$('#meeting_title_'+meeting_id).attr("onmouseover","this.style.backgroundColor='#eee'");
			$('#meeting_title_'+meeting_id).attr("onmouseout","this.style.backgroundColor=''");
		}
		
		$('#meeting_details_holder_'+meeting_id).slideUp('fast');
		$('#meeting_expanded_'+meeting_id).val(0);
	}
}


function expand_collapse_batch(batch_id){
	if($('#batch_expanded_'+batch_id).val() == 0){
		$('#batch_title_'+batch_id).css('background-color','#76a5af');
		$('#batch_title_'+batch_id).css('color','#fff');
		$('#batch_title_'+batch_id).attr("onmouseover","this.style.backgroundColor='#668c94'");
		$('#batch_title_'+batch_id).attr("onmouseout","this.style.backgroundColor='#76a5af'");
		
		$('#batch_details_holder_'+batch_id).slideDown('fast');
		$('#batch_expanded_'+batch_id).val(1);
		
	}else{
		if($('#batch_processed_'+batch_id).val() == 1){
			$('#batch_title_'+batch_id).css('background-color','#c2eae9');
			$('#batch_title_'+batch_id).css('color','#000');
			
			$('#batch_title_'+batch_id).attr("onmouseover","this.style.backgroundColor='#b2dad9'");
			$('#batch_title_'+batch_id).attr("onmouseout","this.style.backgroundColor='#c2eae9'");
			
		}else{
			$('#batch_title_'+batch_id).css('background-color','');
			$('#batch_title_'+batch_id).css('color','#000');
			
			$('#batch_title_'+batch_id).attr("onmouseover","this.style.backgroundColor='#eee'");
			$('#batch_title_'+batch_id).attr("onmouseout","this.style.backgroundColor=''");
		}
		
		$('#batch_details_holder_'+batch_id).slideUp('fast');
		$('#batch_expanded_'+batch_id).val(0);
	}
}

function meeting_all_selection(meeting_id,new_status){
	var meeting_participant_string = $('#participant_string_'+meeting_id).val();
	
	var meeting_participant_array = meeting_participant_string.split(',');
	
	for(var p=0;p<meeting_participant_array.length;p++){
		if($('#participant_selected_'+meeting_participant_array[p]).val() == 0){
			
			if(new_status == 1){
				if($('#participant_active_'+meeting_participant_array[p]).val() == 1){
					document.getElementById('participant_check_input_'+meeting_participant_array[p]).checked = true;
					add_to_selection(meeting_participant_array[p],'selected_participants_'+meeting_id);
					$('#participant_selected_'+meeting_participant_array[p]).val(1);
				}
			}
			...
			if(new_status == 0){
				if($('#participant_active_'+meeting_participant_array[p]).val() == 1){
					document.getElementById('participant_check_input_'+meeting_participant_array[p]).checked = false;
					remove_from_selection(meeting_participant_array[p],'selected_participants_'+meeting_id);
					$('#participant_selected_'+meeting_participant_array[p]).val(0);
				}
			}
		}
	}
...
function approve_meeting_participants(meeting_id,action){
	var meeting_participant_string = $('#selected_participants_'+meeting_id).val();
	
	if(meeting_participant_string == ''){
		alert('You need to select at-least one participant');
		...
		if(c){
			...
			var data = new FormData();
			data.append('approve_meeting_participants',1);
			data.append('meeting_id',meeting_id);
			data.append('participant_string',$('#selected_participants_'+meeting_id).val());
			data.append('action',action);
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#approve_meeting_participants_button_'+meeting_id).html('Wait...');
			$('#approve_meeting_participants_button_1_'+meeting_id).html('Wait...');
			
			$('#disapprove_meeting_participants_button_'+meeting_id).html('Wait...');
			$('#disapprove_meeting_participants_button_1_'+meeting_id).html('Wait...');
		}
	}
}


function delete_meeting_participants(meeting_id){
	var meeting_participant_string = $('#selected_participants_'+meeting_id).val();
	...
	if(meeting_participant_string == ''){
		...
		if(c){
			...
			var data = new FormData();
			data.append('delete_meeting_participants',1);
			data.append('meeting_id',meeting_id);
			data.append('participant_string',$('#selected_participants_'+meeting_id).val());
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#delete_meeting_participants_button_'+meeting_id).html('Wait...');
			$('#delete_meeting_participants_button_1_'+meeting_id).html('Wait...');
		}
	}
}

function export_meeting_list(){
	var data = new FormData();
	data.append('export_meeting_list',1);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
	...
	$('#export_meeting_button').html('Wait...');
	$('#export_participant_button').html('Wait...');
}

function export_participant_list(){
	var data = new FormData();
	data.append('export_participant_list',1);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
...
function fetch_ussd_loader(){
	
	if($('#c_id').val() == '' || $('#ussd_string').val() == ''){
		alert('You need to enter the phone number and the USSD string');
		
	}else{
		var data = new FormData();
		data.append('fetch_ussd_loader',1);
		data.append('c_id',$('#c_id').val());
		data.append('ussd_string',$('#ussd_string').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#ussd_response').html('<div style="width:100%;height:25px;line-height:25px;float:left;margin-top:20px;float:left;text-align:center;">Waiting for USSD response</div>');
	}
	
}

function download_participant_claim_form(meeting_id){
	window.open($('#url').val()+'/meeting_claim/?m='+meeting_id+'&c='+$('#company_id').val(),'claim_form');
}

function batch_meetings(){
	
	if($('#selected_meetings').val() == ''){
		alert('You need to select atleast one meeting');
		
	}else{
		var data = new FormData();
		data.append('batch_meetings',1);
		data.append('selected_meetings',$('#selected_meetings').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#item_details_title').html('Batch meeting participants for payment');
		show_window('item_details',1);
...
function process_batch_meetings(){
	if($('#batch_meetings_button').html() != 'Wait...'){
		$('#batch_error').slideUp('fast');
		
		if($('#total_approved').val() == 0){
			$('#batch_error').slideDown('fast');
			$('#batch_error').html('You need to have atleast one approved participant in the selected meetings or teams');
		
		}else if($('#batch_title').val() == 'Enter batch title here'){
			$('#batch_error').slideDown('fast');
			$('#batch_error').html('You need to enter a batch title. This could be a district name where the meetings or teams belong');
			
		}else if($('#selected_meeting_claim').val() == 0){
			$('#batch_error').slideDown('fast');
			$('#batch_error').html('You need to pick a claim type to use for paying participants in selected meetings or teams');
			...
			if(c){
				var data = new FormData();
				data.append('process_batch_meetings',1);
				data.append('batch_title',$('#batch_title').val());
				data.append('claim_types',$('#selected_meeting_claim').val());
				data.append('selected_meeting_ids',$('#selected_meetings').val());
				data.append('total_approved',$('#total_approved').val());
				
				process_simultanious_xmlhttp('module_xmlhttp',data);
				
				$('#batch_meetings_button').html('Wait...');
...
function select_in_view(direction){
	var id_in_view_string = $('#id_in_view_string').val();
	var id_in_view_array = id_in_view_string.split(',');
	
	for(var i=0;i<id_in_view_array.length;i++){
		
		if(direction == 1){
			if(document.getElementById('meeting_check_box_'+id_in_view_array[i]).checked == false){
				$('#meeting_check_box_'+id_in_view_array[i]).click();
			}
		}else{
			if(document.getElementById('meeting_check_box_'+id_in_view_array[i]).checked == true){
				$('#meeting_check_box_'+id_in_view_array[i]).click();
			}
		}
	}
...
function recount_selections(selection_input,selection_holder,output_type){
	var item_count = 0;
	
	if($('#'+selection_input).val() != ''){
		var selected_items = $('#'+selection_input).val();
		var selected_item_array = selected_items.split(',');
		
		item_count = selected_item_array.length;
	}
	
	if(output_type == 0){
		$('#'+selection_holder).val(item_count);
		
	}else if(output_type == 1){
		$('#'+selection_holder).html(item_count);
...
function mark_selected(selection_input,check_box_identifier){
	if($('#'+selection_input).val() != ''){
		var selected_items = $('#'+selection_input).val();
		var selected_item_array = selected_items.split(',');
		
		for(var i=0;i<selected_item_array.length;i++){
			document.getElementById(check_box_identifier+selected_item_array[i]).checked = true;
		}
	}
}

function clear_selections(selection_input,check_box_identifier,selection_holder,output_type){
	var c = confirm('Are you sure you wish to clear all selections?');
	...
	if(c){
		...
		for(var i=0;i<selected_item_array.length;i++){
			document.getElementById(check_box_identifier+selected_item_array[i]).checked = false;
		}
		
		document.getElementById(check_box_identifier+'parent').checked = false;
		$('#'+selection_input).val('');
		
		recount_selections(selection_input,selection_holder,output_type);
		
		
	}
}

function delete_meeting_batch(batch_id){
	var c = confirm('Are you sure you wish to proceed? This action will restore all meetings/teams for this batch');
	
	if(c){
		var data = new FormData();
		data.append('delete_meeting_batch',1);
		data.append('batch_id',batch_id);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);		
		$('#delete_meeting_batch_button').html('Wait...');
	}
}

function edit_batch(batch_id){
	$('#item_details_title').html('Edit meeting/team payment batch');
	show_window('item_details',1);
	change_window_size('item_details',800,500,1);
	show_loading_progress('item_details_holder','Preparing. One moment please.');	
	
	var data = new FormData();
	data.append('edit_batch',1);
	data.append('batch_id',batch_id);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);		
}

function update_batch(batch_id){
	if($('#participant_editing_string').val() == ''){
		alert('There is nothing to update. You have not changed anything on this batch');
		
	}else{
		var c = confirm('Are you sure you wish to proceed?');
		
		if(c){
			var participant_string = $('#participant_editing_string').val();
			var participant_array = participant_string.split(',');
			
			var data = new FormData();
			data.append('update_batch',1);
			data.append('title',$('#batch_title').val());
			data.append('batch_id',batch_id);
			data.append('participant_id_string',$('#participant_editing_string').val());
			for(var p=0;p<participant_array.length;p++){
				data.append('payable_'+participant_array[p],$('#payable_'+participant_array[p]).val());
				data.append('amount_'+participant_array[p],$('#amount_'+participant_array[p]).html());
			}
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#update_batch_button').html('Wait...');
		}
	}
}

function process_batch_attachment(batch_id){
	if($('#uploaded_files').val() == ''){
		alert('You need to add atleast one file');
		
	}else{
		
		var data = new FormData();
		data.append('process_batch_attachment',1);
		data.append('batch_id',batch_id);
...
function remove_batch_file(batch_id,file_src){
	var c = confirm('Are you sure you wish to proceed?');
	
	if(c){
		var data = new FormData();
		data.append('remove_batch_file',1);
		data.append('batch_id',batch_id);
		data.append('file_src',file_src);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
	}
}

function send_batch_to_finance(batch_id){
	if($('#total_batch_files_'+batch_id).val() == 0){
		alert('You first need to attach files to send batch to finance. Attach claim forms and budget to this batch to proceed');
		
	}else{
		var c = confirm('Are you sure you wish to send this batch to finance?');
		
		if(c){
			var data = new FormData();
			data.append('send_batch_to_finance',1);
			data.append('batch_id',batch_id);
			
			$('#send_to_finance_button_'+batch_id).html('Wait...');
...
function fetch_payment_folders(){
	var data = new FormData();
	data.append('fetch_payment_folders',1);
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
	...
	show_loading_progress('agents','Fetching data. Wait...');
}


function fetch_payment_folder_content(folder_id,folder_level){
	var data = new FormData();
	data.append('fetch_payment_folder_content',1);
	
	data.append('region_id',$('#selected_region').val());
	data.append('province_id',$('#selected_province').val());
	data.append('hub_id',$('#selected_hub').val());
	data.append('site_id',$('#selected_site').val());
	data.append('folder_level',folder_level);
	data.append('current_folder_id',folder_id);
	data.append('status',$('#selected_status').val());
	data.append('accessibility_id',$('#selected_accessibility').val());
	data.append('search_key_string',$('#search_key_string').val());
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#payment_folder_holder').html('<div style="width:100%;height:30px;line-height:30px;text-align:center;color:#999;font-size:1.6em;margin-top:30px;">Loading folder content. Wait...</div>');
}

function fetch_payment_folder_details(folder_id,landing_tab){
	var data = new FormData();
	data.append('fetch_payment_folder_details',1);
	data.append('folder_id',folder_id);
	data.append('landing_tab',landing_tab);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#item_details_title').html('Folder details');
	show_window('item_details',1);
	change_window_size('item_details',800,500,1);
	show_loading_progress('item_details_holder','Preparing. One moment please...');
	$('#item_details_title_bar').css('background-color','#c27ba0');
}

function save_or_update_payment_folder(folder_id){
	$('#error_message').slideUp('fast');
	if($('#save_payment_folder_button').html() != 'Wait...'){
		if($('#folder_title').val() == 'Enter folder title here' || $('#folder_title').val() == ''){
			$('#error_message').slideDown('fast');
			$('#error_message').html('You need to put a title for this folder');
			...
			if(c){
				var data = new FormData();
				data.append('save_or_update_payment_folder',1);
				data.append('region_id',$('#selected_folder_region').val());
				data.append('province_id',$('#selected_folder_province').val());
				data.append('hub_id',$('#selected_folder_hub').val());
				data.append('site_id',$('#selected_folder_site').val());
				data.append('folder_id',folder_id);
				data.append('parent_id',$('#current_folder_id').val());
				data.append('folder_level',$('#folder_level').val());
				data.append('title',$('#folder_title').val());
				data.append('accessibility_id',$('#selected_folder_accessibility').val());
				data.append('allow_agents',$('#selected_folder_allow_agents').val());
				data.append('agent_limit',$('#agent_limit').val());
				data.append('budget_limit',$('#budget_limit').val());
				data.append('current_budget_amount',$('#current_budget_amount').val());
				
				
				var total_claim_types = Number($('#total_claim_types').val());

				var claim_type_string = '';
				for(var c=0;c<total_claim_types;c++){
					
					if($('#selected_claim_type_agent_group_'+c).val() != 0 && $('#selected_claim_type_'+c).val() != 0){
						
						if(claim_type_string== ''){
							claim_type_string = $('#selected_claim_type_agent_group_'+c).val()+'-'+$('#selected_claim_type_'+c).val();
				...
				data.append('claim_type_string',claim_type_string);
				
				process_simultanious_xmlhttp('module_xmlhttp',data);
				
				$('#save_payment_folder_button').html('Wait...');
			}
		}
	}
}

function delete_payment_folder(folder_id){
	var c = confirm('Are you sure you wish to proceed?');
	
	if(c){
		var data = new FormData();
		data.append('delete_payment_folder',1);
		data.append('folder_id',folder_id);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#delete_payment_folder_button').html('Wait...');
	}
}

function add_agents_to_folder(){
	if($('#allow_agents').val() == 0){
		alert('This folder does not support agents. Select a folder or create one where you can add agents');
		
	}else{
		var data = new FormData();
		data.append('add_agents_to_folder',1);
		data.append('folder_id',$('#current_folder_id').val());
		data.append('folder_level',$('#folder_level').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#item_details_title').html('Add agents to folder');
		show_window('item_details',1);
		change_window_size('item_details',700,500,1);
		show_loading_progress('item_details_holder','Preparing options. Wait please...');
		$('#item_details_title_bar').css('background-color','#6aa84f');
	}
	
}

function search_folder_agents(){
	var data = new FormData();
	data.append('search_folder_agents',1);
	data.append('search_key_string',$('#search_agent_key_string').val());
	data.append('this_folder_id',$('#this_folder_id').val());
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#search_agents_holder').slideDown('fast');
	$('#agent_folder_search_results').html('<div style="margin-top:10px;width:100%;height:30px;line-height:30px;float:left;text-align:center;font-size:1.5em;">Searching for agents. Wait...</div>');
	
}

function save_add_agents_to_folder(folder_id){
	if($('#selected_folder_agents').val() != ''){
		var data = new FormData();
		data.append('save_add_agents_to_folder',1);
		data.append('folder_id',folder_id);
		data.append('selected_folder_agents',$('#selected_folder_agents').val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#save_add_agent_to_folder_button').html('Wait...');
	
	}
}

function batch_folder_agents(batching_style){
	if($('#current_folder_id').val() != ''){
		
		var data = new FormData();
		data.append('batch_folder_agents',1);
		data.append('folder_id',$('#current_folder_id').val());
		data.append('batching_style',batching_style);
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#item_details_title').html('Create claim from folder');
		show_window('item_details',1);
		change_window_size('item_details',800,500,1);
		show_loading_progress('item_details_holder','Preparing. One moment please...');
		$('#item_details_title_bar').css('background-color','purple');
		
	}
}

function save_folder_claim_data(folder_id){
	if($('#save_folder_claim_data_button').html() != 'Wait...'){
		var c = confirm('Are you sure you wish to proceed?');
		
		if(c){
			var data = new FormData();
			data.append('save_folder_claim_data',1);
			data.append('folder_id',folder_id);
			data.append('month_id',$('#selected_folder_month').val());
			
			var total_claim_types = Number($('#total_claim_types').val());
			
			for(var c=0;c<total_claim_types;c++){
				data.append('claim_type_id_'+c,$('#folder_claim_type_id_'+c).val());
				...
				if($('#claim_agent_string_'+c).val() != ''){
					...
					for(var a =0; a<claim_agents_array.length;a++){
						var agent_date = claim_agents_array[a];
						var agent_days = Number($('#agent_days_'+c+'_'+agent_date).val())
						
						data.append('agent_date_'+c+'_'+a,agent_date);
						data.append('agent_days_'+c+'_'+a,agent_days);
						
					}
					
					data.append('agents_string_'+c,claim_agents);
				}
			}
			
			data.append('total_claim_types',total_claim_types);
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			
			$('#save_folder_claim_data_button').html('Wait...');
			
		}
	}
}

function create_claim_from_folder(folder_id){
	if($('#create_folder_claim_button').html() != 'Wait...'){
		var c = confirm('Are you sure you wish to proceed?');
		
		if(c){
			
			var claim_value = $('#claim_value').html();
			claim_value = claim_value.replace("K","");
			
			var data = new FormData();
			data.append('create_claim_from_folder',1);
			data.append('folder_id',folder_id);
			data.append('month_id',$('#selected_folder_month').val());
			data.append('claim_amount',claim_value);
			
			var total_claim_types = Number($('#total_claim_types').val());
			
			for(var c=0;c<total_claim_types;c++){
				data.append('claim_type_id_'+c,$('#folder_claim_type_id_'+c).val());
				data.append('claim_type_rate_'+c,$('#claim_type_daily_rate_'+c).val());
				var claim_agents = $('#claim_agent_string_'+c).val();
			...
			$('#create_folder_claim_button').html('Wait...');
		}
	}
}

function restore_folder_claim_beneficiaries(folder_id,claim_type_id,c){
	var total_value = 0;

	var total_claim_types = Number($('#total_claim_types').val());
	
	for(var cl=0;cl<total_claim_types;cl++){
		var claim_agents = $('#claim_agent_string_'+cl).val();
		
		if($('#claim_agent_string_'+cl).val() != ''){
			var claim_agents_array = claim_agents.split(',');
			
			for(var a =0; a<claim_agents_array.length;a++){
				
				var this_value = $('#beneficiary_amount_'+cl+'_'+claim_agents_array[a]).html();
	...
	if(claim_balance < 0){
		alert('This claim has exceeded its budget limit. You cannot proceed with this action');
		
	}else{
		var conf = confirm('This will restore the beneficiary list appearing on the claim for this claim type. Proceed?');
		
		if(conf){
			var data = new FormData();
			data.append('restore_folder_claim_beneficiaries',1);
			data.append('folder_id',folder_id);
			data.append('month_id',$('#selected_folder_month').val());
			data.append('folder_id',folder_id);
			data.append('claim_type_id',claim_type_id);
			data.append('claim_type_index',c);
			
			data.append('claim_type_rate',$('#claim_type_daily_rate_'+c).val());
			var claim_agents = $('#claim_agent_string_'+c).val();
			...
			if($('#claim_agent_string_'+c).val() != ''){
				...
				data.append('agents_string',claim_agents);
			}
			
			process_simultanious_xmlhttp('module_xmlhttp',data);
			$('#restore_beneficiaries_button_'+c).html('Wait...');
		}
	}
}