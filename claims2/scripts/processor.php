...
if(isset($_GET['logout'])){
	$cookie = setcookie('session_active',0,time() - 10);
	$cookie = setcookie('user_id',0,time() - 10);
	$cookie = setcookie('user_date',0,time() - 10);
	$cookie = setcookie('activeArea',0,time()-10);
	header('location:'.$main_url.'/?logout');
	
}else{
	//print($_COOKIE['activeArea']);
