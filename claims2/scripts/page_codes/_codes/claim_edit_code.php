...
if($claim_id){
	...
	$this_claim_beneficiary_array = fetch_db_table('claims_connect',$claim_beneficiaries_table,$company_id,'id',' claim_date = '.$this_claim_date);
	
	
	//$agent_data = fetch_db_table('connect','_data',$company_id,'_date','');
	//var_dump($agent_data);
	...
	for($c=0;$c<count($claim_type_date_array);$c++){
		...
		$claim_type_data .= '<div style="width:100%;height:auto;float:left;" id="claim_type_holder_'.$this_claim_type_date.'"><div style="width:100%;margin-top:5px;height:20px;line-height:20px;background-color:#ddf;" id="general_claim_type_title_'.$this_claim_type_date.'">'.$this_claim_type_results['title'].$claim_type_rate.'</div><input type="hidden" id="claim_type_included_'.$this_claim_type_date.'" value="0"><div style="width:100%;height:20px;line-height:20px;background-color:#eef;"><div style="width:125px;height:20px;float:left;margin-right:3px;">Name</div><div style="width:120px;height:20px;float:left;margin-right:3px;">Phone</div><div style="width:100px;height:20px;float:left;margin-right:3px;">NRC</div><div style="width:100px;height:20px;float:left;margin-right:3px;">From</div><div style="width:100px;height:20px;float:left;margin-right:3px;">To</div><div style="width:60px;height:20px;float:left;margin-right:3px;text-align:right;">Worked</div><div style="width:60px;height:20px;float:left;margin-right:3px;text-align:right;">Payable</div><div style="width:70px;height:20px;float:left;margin-right:3px;text-align:right;">Rate(K)</div><div style="width:70px;height:20px;float:left;margin-right:3px;text-align:right;">Amount(K)</div></div><div style="width:100%;height:auto;float:left;" id="claim_type_beneficiaries_holder_'.$this_claim_type_date.'">';
			...
			for($b=0;$b<count($this_claim_beneficiary_array['id']);$b++){
				
				$this_selected_benficiary = $this_claim_beneficiary_array['agent_date'][$b];
				$this_benficiary_id_nrc = $this_claim_beneficiary_array['nrc'][$b];
				
				$this_beneficiary_region_id = $this_claim_beneficiary_array['region_id'][$b];
				$this_beneficiary_province_id = $this_claim_beneficiary_array['province_id'][$b];
				$this_beneficiary_hub_id = $this_claim_beneficiary_array['hub_id'][$b];
				$this_beneficiary_site_id = $this_claim_beneficiary_array['site_id'][$b];
				$this_beneficiary_name = $this_claim_beneficiary_array['_name'][$b];
				
				/*$phone_numbers_2 = mysqli_query($connect,"select * from $phone_number_table where agent_date = '$this_selected_benficiary' and company_id = $company_id")or die(mysqli_error($connect));
					
				$this_agent = mysqli_query($connect,"select * from $agents_table where _date = '$this_selected_benficiary' and company_id = $company_id")or die(mysqli_error($connect));
				$this_agent_results = mysqli_fetch_array($this_agent,MYSQLI_ASSOC);
				
				$this_claim_agent_id = $this_agent_results['id'];
				
				
				$this_claim_beneficiary = mysqli_query($$module_connect,"select * from $claim_beneficiaries_table where agent_date = '$this_selected_benficiary' and claim_date = '$this_claim_date' and type_date = '$this_claim_type_date' and company_id = $company_id")or die(mysqli_error($$module_connect));*/
				
				
				
				if($this_claim_beneficiary_array['type_date'][$b] == $this_claim_type_date){
					...
					$start_date = $this_claim_beneficiary_array['_from'][$b];
					$end_date = $this_claim_beneficiary_array['_to'][$b];
					$days = $this_claim_beneficiary_array['days'][$b];
					
					if($this_claim_beneficiary_array['rate'][$b]){
						$paid_days = $this_claim_beneficiary_array['paid_days'][$b];
						$rate = $this_claim_beneficiary_array['rate'][$b];
					...
					$amount = $this_claim_beneficiary_array['amount'][$b];
					$comment = $this_claim_beneficiary_array['comment'][$b];
					...
					for($p=0;$p<mysqli_num_rows($phone_numbers_2);$p++){
						...
						$phone_number_code = '<option selected>'.$this_claim_beneficiary_array['phone'][$b].'</option>';
					...
					if($this_claim_beneficiary_array['status'][$b] == 3){
					...
					if(!$this_claim_type_results['billing_type']){
						...
						//$agent_data = fetch_db_table('connect','_data',$company_id,'_date','agent_id = '.$this_agent_results['id']);
						$days = $this_claim_beneficiary_array['days'][$b];//check_agent_activity_days($this_claim_agent_id,0,0,$start_date,$end_date,$agent_data,$company_id);
					...
					*/
					$phone_number_code = '<option>'.$this_claim_beneficiary_array['phone'][$b].'</option>';
				...
				if($c==0){
					...
					$days = $days;//check_agent_activity_days($this_agent_results['id'],0,0,$start_date,$end_date,$agent_data,$company_id);
...
?>
<input type="hidden" id="loading_agent_data_status" value="0">
...
for($c=0;$c<mysqli_num_rows($request_types);$c++){
	...
	$new_claim_type_date_array = explode(',',$new_claim_type_date);
	
	for($c=0;$c<count($new_claim_type_date_array);$c++){
		$this_type_date = $new_claim_type_date_array[$c];
		
		$this_request_type = mysqli_query($$module_connect,"select * from request_types where _date = '$this_type_date' and company_id = $company_id")or die(mysqli_error($$module_connect));
		$this_request_type_results = mysqli_fetch_array($this_request_type,MYSQLI_ASSOC);
		
		if(!$this_request_type_results['status']){
			?>
			<input type="hidden" id="claim_billing_type_<?php print($this_type_date);?>" value="<?php print($this_request_type_results['billing_type']);?>">
		
			<input type="hidden" id="claim_day_adjustment_<?php print($this_type_date);?>" value="<?php print($this_request_type_results['day_adjustment']);?>">
			
			<input type="hidden" id="claim_limit_days_<?php print($this_type_date);?>" value="<?php print($this_request_type_results['limit_days']);?>">
			
			<input type="hidden" id="claim_max_days_<?php print($this_type_date);?>" value="<?php print($this_request_type_results['max_days']);?>">
			
			<input type="hidden" id="claim_fixed_amount_<?php print($this_type_date);?>" value="<?php print($this_request_type_results['fixed_amount']);?>">
			<?php
		}		
	}
?>
...
<div style="width:100%;height:20px;line-height:20px;text-align:center;float:left;background-color:#79c7d6;color:#fff;">Agent search results <div class="window_close_button" style="height:20px;line-height:20px;background-color:#aaf" onmouseout="this.style.backgroundColor='#aaf';" onmouseover="this.style.backgroundColor='#9595e5';" onclick="$('#claim_agent_search_holder').slideUp('fast');" id="details_close_button">X</div></div>
	...
	<div style="width:100%;height:30px;float:left;">
		...
		<div style="width:700px;height:auto;margin:0 auto;" id="database_holder">
			<div style="width:90px;height:30px;line-height:30px;float:left;">Search from:</div>
			<div style="width:auto;min-height:30px;height:auto;float:left;">
			...
			<div style="width:auto;height:30px;line-height:30px;float:left;cursor:pointer;"><input type="checkbox" id="search_with_days_select" onchange="if(this.checked){$('#search_with_days_input').val(1);}else{$('#search_with_days_input').val(0);}"><label for="search_with_days_select">Check days worked (slower)</label></div>
			
			<div style="width:80px;height:25px;background-color:#e0bccc;color:#fff;text-align:center;line-height:25px;margin-top:2px;float:left;margin-left:10px;cursor:pointer;" onmouseover="this.style.backgroundColor='#deb7c8';" onmouseout="this.style.backgroundColor='#e0bccc';"  id="agent_search_button" onclick="search_claim_agents();" title="Click to fetch report with specified options">Re-fetch</div>
		</div>
		
		<input type="hidden" id="search_with_days_input" value="0">
