			...
			<div style="width:<?php print($agent_area_width);?>px;height:auto;float:left;">
				...
				for($b=0;$b<count($beneficiary_name_array);$b++){
					...
					if($beneficiary_type_date_array[$b] == $this_claim_type_date){
						...
						$check_ascension_beneficiaries = mysqli_query($$module_connect,"select * from ascension_beneficiaries where agent_date = '$this_beneficiary_agent_date' and claim_date = '$this_claim_date' and type_date = '$this_claim_type_date'")or die(mysqli_error($$module_connect));
...
<div style="width:100%;min-height:40px;height:auto;float:left;background-color:#fff;border-bottom:solid 1px #eee;">
	...
	for($a=0;$a<$this_claim_results['ascensions'];$a++){
		...
		<div style="width:100%;height:auto;float:left;margin-bottom:20px;display:none;" id="spreadsheet_agent_holder_<?php print($claim_date.'_'.$a);?>">
			<div style="width:100%;height:20px;float:left;background-color:#eef"><div style="width:250px;height:20px;float:left;margin-right:3px;">Name</div><div style="width:160px;height:20px;float:left;margin-right:3px;">Claim type</div></div>
					...
					if($asc_ind_array[$a1] == $a){
						...
						for($b=0;$b<count($beneficiary_name_array);$b++){
							if($beneficiary_type_date_array[$b] == $asc_type_date_array[$a1] and $beneficiary_date_array[$b] == $asc_agent_date_array[$a1]){
								...
								?>
								<div style="width:100%;height:20px;float:left;border-bottom:solid 1px #eee;line-height:20px;"><div style="width:250px;height:20px;float:left;margin-right:3px;"><?php print($this_asc_ben_name);?></div><div style="width:190px;height:20px;float:left;margin-right:3px;"><?php print($this_claim_type_results['title']);?></div></div>
		...
		<?php
			...
			for($sc=0;$sc<count($spreadsheet_claim_type_array);$sc++){
				?>
				<input type="hidden" id="spreadsheet_<?php print($this_claim_results['claim_id'].$alphabet[$a].'_'.$spreadsheet_claim_type_array[$sc]);?>" value="<?php print($spreadsheet_beneficiary[$this_claim_results['claim_id'].$alphabet[$a]][$spreadsheet_claim_type_array[$sc]]);?>">
				<?php
			}
