...
<div style="width:975px;min-height:300px;max-height:500px;float:left;overflow:auto;background-color:#fff">
	...
	for($u=0;$u<mysqli_num_rows($users);$u++){
		...
		$user_id_array[$u] = $this_user_results['id'];
	...
	$user_allocation_array = fetch_db_table('claims_connect','claim_user_allocations',$company_id,'_date desc',' claim_date = '.$this_claim_results['_date']);
	?>
	<div style="width:100%;min-height:20px;height:auto;float:left;background-color:#f3f8fd;margin-bottom:5px;">
	<div style="width:100%;height:20px;line-height:20px;float:left;text-align:center;cursor:pointer;" onmouseover="this.style.backgroundColor='#eef'" onmouseout="this.style.backgroundColor='';" onclick="$('#user_allocations_<?php print($this_claim_results['id']);?>').slideToggle('fast');" title="Click to view users assigned to work on this claim">User allocations</div>
	
	<div style="width:100%;height:auto;float:left;display:none;" id="user_allocations_<?php print($this_claim_results['id']);?>">
	<div style="width:100%;height:20px;line-height:20px;float:left;font-weight:bold;border-bottom:solid 1px #eee;">
		<div style="width:70px;height:20px;line-height:20px;float:left;">Level</div>
		<div style="width:180px;height:20px;line-height:20px;float:left;">Allocated to</div>
		<div style="width:180px;height:20px;line-height:20px;float:left;">Allocated by</div>
		<div style="width:120px;height:20px;line-height:20px;float:left;">Date allocated</div>
		<div style="width:120px;height:20px;line-height:20px;float:left;">Time allocated</div>
		<div style="width:80px;height:20px;line-height:20px;float:left;">Status</div>
	
	</div>
	<?php 
	if(!count($user_allocation_array['id'])){
	?>	
		<div style="width:100%;height:20px;line-height:20px;color:red;text-align:center;float:left;font-weight:bold;">This claim is not allocated to any user</div>
		<?php
	}else{
		?>
		<div style="width:100%;min-height:20px;height:auto;float:left;background-color:#fffdf6;margin-bottom:5px;">
		<?php
		for($al=0;$al<count($user_allocation_array['id']);$al++){
			$allocated_to_index = array_keys($user_id_array,$user_allocation_array['allocated_to_id'][$al]);
			
			$allocated_to_name = $user_name_array[$allocated_to_index[0]];
			
			$allocated_by_index = array_keys($user_id_array,$user_allocation_array['user_id'][$al]);
			
			$allocated_by_name = $user_name_array[$allocated_by_index[0]];
			
			$status = 'Active';
			$color = '#000';
			if(!$user_allocation_array['status'][$al]){
				$status = 'Inactive';
				$color = '#aaa';
			}
			?>
			<div style="width:100%;height:20px;line-height:20px;float:left;border-bottom:solid 1px #eee;color:<?php print($color);?>">
				<div style="width:70px;height:20px;line-height:20px;float:left;"><?php print($user_allocation_array['_level'][$al]+1);?></div>
				<div style="width:180px;height:20px;line-height:20px;float:left;"><?php print($allocated_to_name);?></div>
				<div style="width:180px;height:20px;line-height:20px;float:left;"><?php print($allocated_by_name);?></div>
				<div style="width:120px;height:20px;line-height:20px;float:left;"><?php print(date('jS M, Y',$user_allocation_array['_date'][$al]));?></div>
				<div style="width:120px;height:20px;line-height:20px;float:left;"><?php print(date('H:i:s',$user_allocation_array['_date'][$al]));?></div>
				<div style="width:80px;height:20px;line-height:20px;float:left;"><?php print($status);?></div>
			</div>
	...
	$request_type_date_array = array();
	$request_type_title_array = array();
	...
	for($c=0;$c<count($claim_type_array);$c++){
		...
		$request_type_date_array[count($request_type_date_array)] = $this_claim_type_date;
		$request_type_title_array[count($request_type_title_array)] = $this_claim_type_results['title'];
	...
	$check_approval = mysqli_query($$module_connect,"select * from $claim_approvals_table where claim_date = '$this_claim_date' and company_id = $company_id order by id desc")or die(mysqlI_error($$module_connect));
	...
	<div style="min-width:100%;width:<?php print($total_claim_width);?>px;min-height:30px;height:auto;float:left;">		
		<?php
		$table_claim_beneficiaries_array = fetch_db_table('claims_connect',$claim_beneficiaries_table,$company_id,'id','');
		$table_claim_ascensions_array = fetch_db_table('claims_connect',$claim_ascension_table,$company_id,'id','claim_date = '.$claim_date);
		...
		$ascension_beneficiaries = mysqli_query($$module_connect,"select * from $claim_ascension_table where claim_date = '$claim_date' and company_id = $company_id")or die(mysqli_error($$module_connect));
		...
		$this_claim_benefificiary_index = array_keys($table_claim_beneficiaries_array['claim_date'],$claim_date);
		...
		for($c=0;$c<count($claim_type_array);$c++){
			...
			<div style="width:<?php print($agent_area_width);?>px;height:auto;float:left;">
				...
				<?php
				$total_amount = 0;
				if(isset($this_claim_benefificiary_index[0])){
					for($b=0;$b<count($this_claim_benefificiary_index);$b++){
						
						if($table_claim_beneficiaries_array['type_date'][$this_claim_benefificiary_index[$b]] == $this_claim_type_date){
							$has_beneficiaries = 1;
							$this_beneficiary_name = $table_claim_beneficiaries_array['_name'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_phone = $table_claim_beneficiaries_array['phone'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_nrc = $table_claim_beneficiaries_array['nrc'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_days = $table_claim_beneficiaries_array['days'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_paid_days = $table_claim_beneficiaries_array['paid_days'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_rate = $table_claim_beneficiaries_array['rate'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_amount = $table_claim_beneficiaries_array['amount'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_comment = $table_claim_beneficiaries_array['comment'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_from = $table_claim_beneficiaries_array['_from'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_to = $table_claim_beneficiaries_array['_to'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_level = $table_claim_beneficiaries_array['level'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_id = $table_claim_beneficiaries_array['id'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_agent_date = $table_claim_beneficiaries_array['agent_date'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_region_id = $table_claim_beneficiaries_array['region_id'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_province_id = $table_claim_beneficiaries_array['province_id'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_hub_id = $table_claim_beneficiaries_array['hub_id'][$this_claim_benefificiary_index[$b]];
							$this_beneficiary_site_id = $table_claim_beneficiaries_array['site_id'][$this_claim_benefificiary_index[$b]];
							
							$this_beneficiary_date = $this_beneficiary_agent_date;
							$this_beneficiary_status = $table_claim_beneficiaries_array['status'][$this_claim_benefificiary_index[$b]];;
							
							
							$this_beneficiary_region_index = array_keys($region_id_array,$this_beneficiary_region_id);
							
							$this_beneficiary_province_index = array_keys($province_id_array,$this_beneficiary_province_id);
							
							$this_beneficiary_hub_index = array_keys($hub_id_array,$this_beneficiary_hub_id);
							
							$this_beneficiary_site_index = array_keys($site_id_array,$this_beneficiary_site_id);
							...
							for($a=0;$a<count($approval_beneficiary_date_array);$a++){
								if($approval_beneficiary_date_array[$a] == $this_beneficiary_date and !$approval_status_array[$a] and $approval_claim_type_date_array[$a] == $this_claim_type_date and $approval_validity_array[$a] ==1){
							...
							$ascension_index = array_keys($table_claim_ascensions_array['agent_date'],$this_beneficiary_agent_date);
							
							$check_disabled = 0;
							$spreadsheet_number = '';
							if(isset($ascension_index[0])){
								
								for($ab=0;$ab<count($ascension_index);$ab++){
									if($table_claim_ascensions_array['type_date'][$ascension_index[$ab]] == $this_claim_type_date){
										$check_disabled = 1;
										$spreadsheet_number = $this_claim_results['claim_id'].$alphabet[$table_claim_ascensions_array['ascension_ind'][$ascension_index[$ab]]];
										break;
									}
								}
							}
							
							/*$check_ascension_beneficiaries = mysqli_query($$module_connect,"select * from $claim_ascension_table where agent_date = '$this_beneficiary_agent_date' and claim_date = '$this_claim_date' and type_date = '$this_claim_type_date' and status = 1")or die(mysqli_error($$module_connect));
							...
							if($beneficiary_active and !$check_disabled){
								if($selected_beneficiaries == ''){
									$selected_beneficiaries = $this_beneficiary_date;
							...
							if($all_beneficiaries == ''){
								$all_beneficiaries = $this_beneficiary_date;
							...
							<div style="width:100%;float:left;height:20px;border-bottom:solid 1px #eee;position:relative" onmouseover="this.style.backgroundColor='#eee';" onmouseout="this.style.backgroundColor='';">
								...
								print(ucwords(strtolower($beneficiary_name)));?></div>
								...
								<?php
								if($this_beneficiary_status == 0){
									$beneficiary_status = 'Disabled';
									
								}else if($this_beneficiary_status == 1){
									$beneficiary_status = 'Pending';
									
								}else if($this_beneficiary_status == 2){
									$beneficiary_status = 'Completed';
									
								}else if($this_beneficiary_status == 3){
									$beneficiary_status = 'Rejected';
									
								}else{
									$beneficiary_status = $this_beneficiary_status;								
								}
								?>
								...
								NRC on claim: <?php print($this_beneficiary_nrc.'<br>');
								print('Region: '.$this_beneficiary_region.', Province: '.$this_beneficiary_province.', Hub: '.$this_beneficiary_hub.', Site: '.$this_beneficiary_site);?><br>
					...
					<div style="width:<?php print($cell_width);?>px;height:auto;float:left;">
						...
						if(!$this_level_action_type){
							...
							for($b=0;$b<count($this_claim_benefificiary_index);$b++){
								...
								$this_beneficiary_type_date = $table_claim_beneficiaries_array['type_date'][$this_claim_benefificiary_index[$b]];
								
								$this_beneficiary_agent_date = $table_claim_beneficiaries_array['agent_date'][$this_claim_benefificiary_index[$b]];
								if($this_beneficiary_type_date == $this_claim_type_date){
									
									$level_waiting = 0;
									for($a=0;$a<count($approval_beneficiary_date_array);$a++){
										if($approval_beneficiary_date_array[$a] == $this_beneficiary_agent_date and 	$approval_level_array[$a] == $l and $approval_claim_type_date_array[$a] == $this_claim_type_date and $approval_validity_array[$a] ==2){
											$level_waiting = 1;
											
										}
										
										if($approval_beneficiary_date_array[$a] == $this_beneficiary_agent_date and $approval_level_array[$a] == $l and $approval_claim_type_date_array[$a] == $this_claim_type_date and $approval_validity_array[$a] ==1){
											...
											?>
												...
												<div style="width:<?php print($cell_width+80);?>px;height:150px;height:auto;border-bottom:solid 1px #fdd;margin-top:20px;position:absolute;display:none;background-color:#fff;z-index:10;border:solid 1px #ddd;padding:2px;" id="approval_detail_<?php print($this_claim_type_date.'_'.$c.'_'.$this_beneficiary_agent_date.'_'.$l);?>" ondblclick="$(this).slideUp('fast');">
													<div style="width:100%;background-color:#fdd;min-height:20px;height:auto;line-height:20px;float:left;font-weight:bold;text-align:center;"><?php print($this_beneficiary_name.' (Level '.($l+1).')');?></div>
													<div style="width:100%;min-height:150px;max-height:200px;float:left;overflow:auto;font-size:0.9em;line-height:15px;">
													<?php
													
													$this_beneficiary_approval_index = array_keys($approval_beneficiary_date_array,$this_beneficiary_agent_date);
								...
								for($d=0;$d<count($document_string);$d++){
									if(!$approved){
										...
										if(!$approval_status){
											...
											if($this_claim_results['creation_method'] and $this_claim_results['title'] == ''){
												print('<a title="Click to open file" style="cursor:pointer;" onmouseover="this.style.color=\'orange\'" onmouseout="this.style.color=\'\'" onclick="window.open(\''.$main_url.'/imgs/'.$document_string[$d].'\',\'file_download_'.$d.'\')">'.($d+1).'. '.$file_name.'</a><br><br>');
																...
																if($approval_file_src_array[$this_beneficiary_approval_index[$tb]] != ''){
																	...
																	for($f2=0;$f2<count($approval_files);$f2++){
																		...
																		if(strlen($this_file_name)>10){
																			$this_file_name =  substr($this_file_name,0,10).'...';
																		...
																		if($approval_file_string == ''){
																			...
																			$approval_file_string = '<a title="Click to open file" style="cursor:pointer;" onmouseover="this.style.color=\'orange\'" onmouseout="this.style.color=\'\'" onclick="window.open(\''.$item_url.'/imgs/'.$approval_files[$f2].'\',\'file_download\')">'.($f2+1).'. '.$this_file_name.'</a><br>';
...
<div style="width:100%;min-height:40px;height:auto;float:left;background-color:#fff;border-bottom:solid 1px #eee;">
	<?php
		if($this_claim_results['file_src'] != ''){
			...
			for($f=0;$f<count($attachments);$f++){
				...
				$item_url = $url;
				if($this_claim_results['creation_method']){
					//$item_url = $main_url;
				...
				if($this_claim_results['claim_id'] > 5290){
					...
					$file_location = $item_url.'imgs/attachments/'.$attachments[$f];
	...
	<div style="width:100px;height:auto;float:left;margin-top:5px;position:relative">
		<div style="width:auto;min-height:30px;height:auto;float:left;">
			...
			<div class="option_menu" id="claim_form_menu_<?php print($this_claim_date);?>" style="display:none;min-width:250px;color:#fff;background-color:#35bcc7;">
				...
				<div class="option_menu_item" onmouseover="this.style.backgroundColor='#56cfd8';" onmouseout="this.style.backgroundColor='#35bcc7';" title="Click to download claim form for signing" onclick="$('#claim_form_menu_<?php print($this_claim_date);?>').toggle('fast');open_spreadsheet(<?php print($this_claim_date.','.$user_date.','.$company_id);?>,5,0,'','')">For CHWs (Separate claim type sheets)</div>
	...
	?>
		...
		<div style="background-color:orange;width:70px;color:#fff;cursor:pointer;height:30px;margin:5px;float:right;text-align:center;line-height:30px;border:solid 1px #fff;" onmouseover="this.style.backgroundColor='#ffc864';" onmouseout="this.style.backgroundColor='orange';" onclick="fetch_claim_details(<?php print($this_claim_results['_date']);?>);">Edit</div>
	...
	for($a=0;$a<$this_claim_results['ascensions'];$a++){
		...
		<div style="width:100%;height:auto;float:left;margin-bottom:20px;display:none;" id="spreadsheet_agent_holder_<?php print($claim_date.'_'.$a);?>">
			...
			$spreadsheet_claim_types = '';
				for($a1=0;$a1<count($table_claim_ascensions_array['id']);$a1++){
					
					if($table_claim_ascensions_array['ascension_ind'][$a1] == $a){
						for($b=0;$b<count($this_claim_benefificiary_index);$b++){
							...
							$this_beneficiary_agent_name = $table_claim_beneficiaries_array['_name'][$this_claim_benefificiary_index[$b]];
							
							if($this_beneficiary_type_date == $table_claim_ascensions_array['type_date'][$a1] and $this_beneficiary_agent_date == $table_claim_ascensions_array['agent_date'][$a1]){
								
								$this_asc_ben_name = $this_beneficiary_agent_name;
							
								$this_claim_type_date = $table_claim_ascensions_array['type_date'][$a1];
								
								
								$request_type_index = array_keys($request_type_date_array,$this_claim_type_date);
								...
								if($asc_status_array[$a1] == 1){
									if(isset($spreadsheet_beneficiary[$this_claim_results['claim_id'].$alphabet[$a]][$this_claim_type_date])){
										...
										$spreadsheet_beneficiary[$this_claim_results['claim_id'].$alphabet[$a]][$this_claim_type_date] = $table_claim_ascensions_array['agent_date'][$a1];
									}
								}
								...
								?>
	<div style="width:100%;min-height:25px;height:auto;float:left;border-bottom:solid 1px #eee;line-height:25px;<?php if($table_claim_ascensions_array['status'][$a1] == 0){print('color:#aaa;');}?>" id="ascension_beneficiary_holder_<?php print($claim_date.'_'.$a.'_'.$this_beneficiary_agent_date.'_'.$this_claim_type_date);?>"> <div style="width:20px;height:25px;float:left;"><?php if($user_date == $this_user_date || $active_user_roles[8] == 1){?><div style="cursor:pointer;color:#fff;margin-top:2px;margin-right:5px;width:20px;height:20px;float:left;background-color:#74b474;text-align:center;line-height:20px;<?php if($table_claim_ascensions_array['status'][$a1] != 0){?>display:none;<?php }?>" onmouseover="this.style.backgroundColor='#9ec39e'" onmouseout="this.style.backgroundColor='#74b474';" onclick="change_status_spreadsheet_beneficiary(<?php print($claim_date.','.$a.','.$this_beneficiary_agent_date.','.$this_claim_type_date.',1,\''.$this_claim_results['claim_id'].$alphabet[$a].'\'');?>);" id="ascension_beneficiary_add_<?php print($claim_date.'_'.$a.'_'.$this_beneficiary_agent_date.'_'.$this_claim_type_date);?>">&#10003;</div> <div style="cursor:pointer;color:#fff;margin-top:2px;margin-right:5px;width:20px;height:20px;float:left;background-color:brown;text-align:center;line-height:20px;<?php if($table_claim_ascensions_array['status'][$a1] == 0){?>display:none;<?php }?>" onmouseover="this.style.backgroundColor='#b83f3f'" onmouseout="this.style.backgroundColor='brown';" onclick="change_status_spreadsheet_beneficiary(<?php print($claim_date.','.$a.','.$this_beneficiary_agent_date.','.$this_claim_type_date.',0,\''.$this_claim_results['claim_id'].$alphabet[$a].'\'');?>);" id="ascension_beneficiary_remove_<?php print($claim_date.'_'.$a.'_'.$this_beneficiary_agent_date.'_'.$this_claim_type_date);?>">X</div><?php }?></div><div style="width:250px;min-height:25px;height:auto;float:left;margin-right:3px;"><?php print($this_asc_ben_name);?></div><div style="width:250px;min-height:25px;height:auto;float:left;margin-right:3px;"><?php print($request_type_title_array[$request_type_index[0]]);?></div><div style="width:200px;min-height:25px;height:auto;float:left;margin-right:3px;" id="ascension_beneficiary_status_<?php print($claim_date.'_'.$a.'_'.$this_beneficiary_agent_date.'_'.$this_claim_type_date);?>"></div></div>
