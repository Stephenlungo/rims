...
module_xmlhttp.onreadystatechange = function(){
	if(module_xmlhttp.readyState == 4 && module_xmlhttp.status == 200){
		...
		if(response_array[0] == 'session_expired'){	
		...
		}else if(response_array[0] == 'fetch_claim_details' || response_array[0] == 'allocate_claims' || response_array[0] == 'merge_claims'){
			display_infor('item_details_holder',response_array[1]);
			
		}else if(response_array[0] == 'complete_allocate_user'){
			alert('Operation successfuly...');
			close_window('item_details',1);
			//select_or_deselect_all_claims(0);
			...
			if(response_array[6] != ''){
				...
				for(var f =0;f<file_array.length;f++){
					if(file_string_2 == ''){
						...
						file_string_2 = '<a title="Click to open file" style="cursor:pointer;" onmouseover="this.style.color=\'orange\'" onmouseout="this.style.color=\'\'" onclick="window.open(\''+$('#claims_url').val()+'/imgs/'+file_array[f]+'\',\'file_download\')">'+file_array[f]+'</a>';
						
						
					}else{
						file_string_2 = file_string+'<br><a title="Click to open file" style="cursor:pointer;" onmouseover="this.style.color=\'orange\'" onmouseout="this.style.color=\'\'" onclick="window.open(\''+$('#claims_url').val()+'/imgs/'+file_array[f]+'\',\'file_download\')">'+file_array[f]+'</a>';
			...
			$('#active_claim_export').html('Export');
			window.open($('#claims_url').val()+'/'+response_array[2]);
		...
		}else if(response_array[0] == 'search_creator_users'){
			$('#creator_search_results_holder').html(response_array[1]);
		...
		}else if(response_array[0] == 'export_claims'){
			$('#active_claim_export').html('Export');
			window.open($('#claims_url').val()+'/'+response_array[1],'claim_export');
			
		}else if(response_array[0] == 'fetch_edit_beneficiary_data'){
			var claim_date = response_array[1];
			var claim_type_date = response_array[2];
			var agent_date = response_array[3];
			
			var agent_id = response_array[4];
			var agent_name = response_array[5];
			var agent_nrc = response_array[6];
			var agent_phone = response_array[7];
			
			$('#beneficiary_item_'+claim_type_date+'_'+agent_date).css('color','#ce7e00');
			
			$('#claim_agent_name_'+claim_type_date+'_'+agent_date).html(agent_name);
			$('#claim_agent_phone_'+claim_type_date+'_'+agent_date).html(agent_phone);
			$('#claim_agent_nrc_'+claim_type_date+'_'+agent_date).html(agent_nrc);
			
			$('#agent_start_date_'+claim_type_date+'_'+agent_date).attr('onchange','include_fetch_agent_days(\'_'+claim_type_date+'\',\'_'+agent_date+'\','+agent_id+')');
			
			$('#agent_end_date_'+claim_type_date+'_'+agent_date).attr('onchange','include_fetch_agent_days(\'_'+claim_type_date+'\',\'_'+agent_date+'\','+agent_id+')');
			
			$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).slideUp('fast');
			
			$('#loading_agent_data_status').val(0);
			
		}else if(response_array[0] == 'open_spreadsheet'){
			fetch_claim_beneficiaries(response_array[1],1);
			
			window.open($('#claims_url').val()+'/spreadsheet.php?s=true&asc='+response_array[2]+'&asc_type=2&k='+response_array[4]+'&t='+response_array[5]+'&ci='+response_array[1]+'&comp='+$('#company_id').val()+'&u='+response_array[6]+'&b='+response_array[7],'spreadsheet');
...
function fetch_payment_claims(){
	...
	data.append('creator_id',$('#selected_creator').val());
	data.append('finance_processed',$('#selected_finance_processed').val());
	...
	data.append('creation_method',$('#selected_creation_method').val());
	
	data.append('ordering',$('#selected_ordering').val());
	data.append('user_allocation',$('#selected_allocation').val());
	data.append('allocation_colors',$('#selected_allocation_colors').val());
...
function open_spreadsheet(claim_id,user_date,company_id,_type,ascensions,spreadsheet_id,spreadsheet_holders){
	...
	if(_type==4){	
		window.open($('#claims_url').val()+'/spreadsheet2.php?s=true&asc='+ascensions+'&asc_type='+spreadsheet_id+'&k='+key+'&t='+_type+'&ci='+claim_id+'&comp='+company_id+'&u='+user_date+'&b='+beneficiary_string,'spreadsheet');
		
	}else{
		window.open($('#claims_url').val()+'/spreadsheet.php?s=true&asc='+ascensions+'&asc_type='+spreadsheet_id+'&k='+key+'&t='+_type+'&ci='+claim_id+'&comp='+company_id+'&u='+user_date+'&b='+beneficiary_string,'spreadsheet');
...
function open_spreadsheet_2(claim_id,user_date,company_id,_type,ascensions,spreadsheet_id,spreadsheet_holders){
	...
	var data = new FormData();
	data.append('open_spreadsheet',1);
	data.append('claim_date',claim_id);
	data.append('user_date',user_date);
	data.append('_type',_type);
	data.append('ascensions',ascensions);
	data.append('spreadsheet_id',spreadsheet_id);
	data.append('key',key);
	data.append('beneficiary_string',beneficiary_string);
	
	process_simultanious_xmlhttp('module_xmlhttp',data);
	
	$('#active_spreadsheet_'+claim_id).html('Wait...');
}
...
function search_claim_agents(){
	...
	if($('#selected_new_region').val() == 0){
		...
		if($('#claim_agent_search_input').val() == 'Enter agent name or phone number' || $('#claim_agent_search_input').val() == ''){
			...
			data.append('database',$('#selected_database').val());
			data.append('search_with_days',$('#search_with_days_input').val());
...
function process_approval_files(claim_date,claim_type_date,level_index){
	...
	for(var f=0;f<added_files_array.length;f++){
		...
		addition_div = addition_div+'<br><a title="Click to open file" style="cursor:pointer;" onmouseover="this.style.color=\'orange\'" onmouseout="this.style.color=\'\'" onclick="window.open(\''+$('#claims_url').val()+'/imgs/'+added_files_array[f]+'\',\'file_download\')">'+(f+1)+'. '+added_file_name+'</a><br>';
				...
				if(c){
					...
					for(var c = 0;c<selected_claim_types_array.length;c++){
						...
						for(var b=0;b<selected_beneficiaries_array.length;b++){
							...
							data.append('beneficiary_region_'+this_claim_date+'_'+this_beneficiary_date,$('#beneficiary_region_'+this_claim_date+'_'+this_beneficiary_date).val());
							data.append('beneficiary_province_'+this_claim_date+'_'+this_beneficiary_date,$('#beneficiary_province_'+this_claim_date+'_'+this_beneficiary_date).val());
							data.append('beneficiary_hub_'+this_claim_date+'_'+this_beneficiary_date,$('#beneficiary_hub_'+this_claim_date+'_'+this_beneficiary_date).val());
							data.append('beneficiary_site_'+this_claim_date+'_'+this_beneficiary_date,$('#beneficiary_site_'+this_claim_date+'_'+this_beneficiary_date).val());
							data.append('beneficiary_name_'+this_claim_date+'_'+this_beneficiary_date,$('#beneficiary_name_'+this_claim_date+'_'+this_beneficiary_date).val());
						}
					}
...
function fetch_edit_beneficiary_data(claim_date,claim_type_date,agent_date){
	if($('#loading_agent_data_status').val() == 1){
		alert('Wait for the agent data being loaded to finish before loading this one');
		
	}else{
		var data = new FormData();
		
		data.append('fetch_edit_beneficiary_data',1);
		data.append('claim_date',claim_date);
		data.append('claim_type_date',claim_type_date);
		data.append('agent_date',agent_date);
		data.append('phone_number',$('#claim_agent_phone_'+claim_type_date+'_'+agent_date).val());
		
		process_simultanious_xmlhttp('module_xmlhttp',data);
		
		$('#loading_agent_data_status').val(1);
		
		$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).html('Reading agent records. Wait...');
		$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).css('opacity','0.9');
		$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).css('text-align','center');
		$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).css('background-color','#fff');
		$('#entry_edit_fade_'+claim_type_date+'_'+agent_date).css('border','solid 1px #006bb3');
	}
}
...
function confirm_level(claim_date,claim_type_date,beneficiary_date,claim_type_index,level_index,request_type,action_type,all_or_single){
	
	if(all_or_single == undefined){
		var all_or_single = 0;
				...
				if($('#approval_queue').val() == ''){
					$('#approval_queue').val(claim_date+'}'+claim_type_date+'}'+beneficiary_date+'}'+claim_type_index+'}'+level_index+'}'+$('#file_'+claim_date+'_'+claim_type_date+'_'+level_index).val());
						...
						if(all_or_single == 0){
							...
							var act = setTimeout("process_level_confirm_queue_all()",1000);
						}
					*/
...
function process_level_confirm_queue_all(){
	if($('#approval_queue').val() != ''){
		
		var data = new FormData();
		data.append('process_level_confirm_queue_all',1);
		data.append('approval_code',$('#approval_queue').val());
		data.append('total_in_queue',approval_code_array.length);
	...
	if(c){
		...
		for(var b=0;b<selected_beneficiaries_array.length;b++){
			if($('#level_approved_'+claim_date+'_'+claim_type_date+'_'+selected_beneficiaries_array[b]+'_'+level_index).val() == 0){
				confirm_level(claim_date,claim_type_date,selected_beneficiaries_array[b],claim_type_index,level_index,1,1,0);
			}
		}
...
function fetch_awaiting_beneficiaries(awaiting_date,type){
...
function search_creator_users(){
	var data = new FormData();
	data.append('search_creator_users',1);
	$('#creator_search_result_holder').slideDown('fast');
	data.append('search_key',$('#creator_users_search_key').val());
	
	process_simultanious_xmlhttp('module_xmlhttp',data);	
	
	$('#creator_search_results_holder').html('<div style="width:100%;height:25px;line-height:25px;float:left;text-align:center;margin-top:20px;">Searching users. Wait...</div>');
}


function select_or_deselect_all_claims(new_status){
	if($('#claims_in_view').val() != ''){
		var all_claims = $('#claims_in_view').val();
		
		var all_claims_array = all_claims.split(',');
		
		for(var i=0;i<all_claims_array.length;i++){
		
			if(new_status == 0){
				if($('#claim_'+all_claims_array[i]+'_selected').val() == 1){
					$('#claim_check_'+all_claims_array[i]).click();
					
				}
				
			}else{
				if($('#claim_'+all_claims_array[i]+'_selected').val() == 0){
					$('#claim_check_'+all_claims_array[i]).click();
...
function allocate_claims(){
	if($('#selected_claims').val() == ''){
		alert('You need to select atleast 1 claim to allocate to a user');
		...
		$('#item_details_title').html('Allocate claims to user');
		
		var data = new FormData();
		data.append('allocate_claims',1);
		data.append('select_claims',$('#selected_claims').val());
...
function merge_claims(){
	if($('#claims_merge_button').html() != 'Wait...'){
		var selected_claims = $('#selected_claims').val();
		var selected_claims_array = selected_claims.split(',');
		
		if($('#selected_claims').val() == '' || selected_claims_array.length < 2){
			alert('You need to select atleast 2 claims to proceed with merge');
			...
			$('#item_details_title').html('Merge claims');
			
			var data = new FormData();
			data.append('merge_claims',1);
			data.append('select_claims',$('#selected_claims').val());
...
function complete_allocate_user(){
	if($('#user_allocation_button').html() != 'Wait..'){
		...
		if(c){
			...
			data.append('_level',$('#selected_allocation_level').val());
...
function complete_merge_claims(){
	if($('#claims_merge_button').html() != 'Wait..'){
		
		if($('#selected_target_claim').val() == 0){
			alert('You need to select a target claim');
			
		}else{
			var c = confirm('Are you sure you wish to merge all selected claims into '+$('#active_target_claim').html());
			
			if(c){
				var data = new FormData();
				data.append('complete_merge_claims',1);
				data.append('select_claims',$('#selected_claims').val());
				data.append('target_claim',$('#selected_target_claim').val());
				
				process_simultanious_xmlhttp('module_xmlhttp',data);
				$('#claims_merge_button').html('Wait...');
			}
		}
	}
}

function export_claims(export_status){
	$('#active_claim_export').html('Wait...');
	
	var data = new FormData();
	data.append('export_claims',1);
	data.append('export_status',export_status);
...
function export_tilt_file(){
	if($('#selected_claims').val() == ''){
		alert('You need to select atleast 1 claim to export');
		
	}else{
		var data = new FormData();
		data.append('export_tilt_file',1);
		data.append('select_claims',$('#selected_claims').val());
