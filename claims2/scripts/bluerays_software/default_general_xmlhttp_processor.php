...
if(isset($_POST['process_login'])){
	...
	for($c=0;$c<count($connection);$c++){
		...
		if(mysqli_num_rows($auth)){
			...
			if($auth_results['password'] == $password){
				$output = 1;				$allow_login = 1;
				$message = '';
				$user_id = $auth_results['id'];
				$user_date = $auth_results['_date'];
				
					
				if(($settings_results['limit_attempts'] and ($auth_results['login_attempts'] < $settings_results['allowed_attempts'])) || !$settings_results['limit_attempts']){
...
if(isset($_POST['process_password_reset'])){
	...
	if($check_user_results['reset_code'] == $reset_code){
		$success = 1;
		$error_message = '';
		include 'default_functions.php';
		//$new_password = simple_decode($new_password);
	
		$update_users = mysqli_query($$connection,"update $table_name set password = '$new_password', login_attempts = 0 where id = $this_user_id")or die(mysqli_error($$connection));
	}
	
	print('process_password_reset~'.$this_user_id.'~'.$success.'~'.$error_message);
}
